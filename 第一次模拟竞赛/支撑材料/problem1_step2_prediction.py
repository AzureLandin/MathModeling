...
选择依据：在 R1/R2 中优先 LOOCV-RMSE 更低者；若 R2 同时系数更稳（见第 4 节），则与任务单默认推荐一致，采用对数特征岭回归。
...
def build_recommendation(table4: pd.DataFrame, metrics: dict, table6: pd.DataFrame) -> str:
...
def main() -> dict:
    ...
    rec = build_recommendation(table4, m_best, table6)
