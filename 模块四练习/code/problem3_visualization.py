...
def plot_gap_distribution(models, save_path):
    ...
    ax.axvline(x=np.mean(means), color='orange', linestyle='-', linewidth=2, 
               label=f'均值={np.mean(means):.1f} m$^3$')
    
    ax.set_xlabel(r'偏差均值 $\bar{D}$ (m$^3$)', fontsize=14)
...
def plot_acf_examples(deviation_series, models, plan_ids, save_path, n_examples=3):
    ...
    for k, idx in enumerate(selected):
        ...
        ax.set_title(f'供应商 {plan_ids[idx]}\n{models[idx]["model_type"]}', fontsize=16)
        ax.set_xlabel('滞后阶数', fontsize=15)
        ax.set_ylabel('自相关系数', fontsize=15)
        ax.tick_params(axis='both', labelsize=12)
    
    plt.suptitle('典型供应商偏差序列ACF图', fontsize=22, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def plot_weekly_supply_comparison(weekly_total, save_path, capacity=28200):
    ...
    ax.axhline(y=capacity, color='red', linestyle='--', linewidth=2, label=f'产能要求 ({capacity:,} m$^3$)')
