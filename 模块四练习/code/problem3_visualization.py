...
def plot_model_distribution(models, save_path):
    ...
    ax.set_title('供应商模型类型分布', fontsize=14, pad=20)
...
def plot_gap_distribution(models, save_path):
    ...
    ax.set_xlabel('偏差均值 D̄ (m³)', fontsize=12)
    ax.set_ylabel('供应商数量', fontsize=12)
    ax.set_title('供应商供货偏差均值分布', fontsize=14)
...
def plot_fulfillment_rate(fulfillment_rate, P_j, save_path):
    ...
    ax1.set_title('问题三方案E：24周产能满足率（偏差序列ARIMA）', fontsize=14)
...
def plot_weekly_supply_comparison(weekly_total, save_path, capacity=28200):
    ...
    ax.set_title('问题三方案E：每周供货总量', fontsize=14)
    ax.set_xticks(weeks)
    ax.legend(fontsize=11)
