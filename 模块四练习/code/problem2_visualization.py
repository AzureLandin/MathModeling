...
def plot_weekly_volume(weekly_volume, save_path):
    ...
    ax.set_ylabel('订购量（产品体积 m³）', fontsize=12)
    ax.set_title('24周每周订购量', fontsize=14)
    ax.set_xticks(weeks)
...
def plot_weekly_suppliers(weekly_count, weekly_A, weekly_B, weekly_C, save_path):
    ...
    ax.set_title('每周选中供应商数量（按材料类型）', fontsize=14)
    ax.set_xticks(weeks)
    ax.legend(fontsize=11)
    ax.grid(axis='y', alpha=0.3)
...
def plot_supplier_heatmap(x, ids, types, save_path):
    ...
    im = ax.imshow(x_sorted, cmap='YlGn', aspect='auto', interpolation='nearest')
    
    ax.set_xlabel('周次', fontsize=12)
    ax.set_ylabel('供应商', fontsize=12)
    ax.set_title('供应商订购计划热力图', fontsize=14)
    ...
    plt.colorbar(im, ax=ax, label='是否订购 (0/1)')
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close()
