...
原始数据存在极端值干扰和量纲差异，本文对所有连续变量执行1\%与99\%分位数的Winsorize缩尾处理，再对参与PCA的19个基础变量进行Z-score标准化：
\begin{equation}
z_i = \frac{x_i - \bar{x}}{s}
\label{eq:zscore}
\end{equation}
其中$\bar{x}$为样本均值，$s$为样本标准差。标准化后各变量均值为0、标准差为1，消除了量纲差异对PCA的影响。
...
\lstinputlisting[language=Python, caption={问题一数据清洗与特征提取代码}]{code/problem1_preprocess.py}

\lstinputlisting[language=Python, caption={问题一多元逻辑回归求解代码}]{code/problem1_logit.py}

\lstinputlisting[language=Python, caption={问题一XGBoost辅助验证代码}]{code/problem1_xgboost.py}

\lstinputlisting[language=Python, caption={问题二数据预处理代码}]{code/problem2_preprocess.py}

\lstinputlisting[language=Python, caption={问题二熵权法与加权聚类代码}]{code/problem2_entropy_cluster.py}

\lstinputlisting[language=Python, caption={问题二有监督分类与预测代码}]{code/problem2_classifier.py}

\lstinputlisting[language=Python, caption={问题二随机森林对比代码}]{code/problem2_random_forest.py}
