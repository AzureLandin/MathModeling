...
小微企业因规模相对较小且缺少抵押资产，银行在决定贷款前均要对其信用进行评级。小微企业的信用评级通常基于多个维度，包括但不限于企业的财务状况、经营历史、市场地位、管理能力、行业前景以及企业主的个人信用等，这些因素共同构成了信用评级机构对企业进行信用评估的基础。在实际操作中，银行会主要根据企业的交易票据信息，将其划分为不同的信用等级。
\subsection{问题提出}

题目在附件1提供了123家有信贷记录企业的票据数据，在附件2提供了302家无信贷记录企业的票据数据，要求我们解决以下两个问题：

\textbf{问题一：}根据实际和附件1中的数据信息，通过建立数学模型研究123家企业的信用等级情况的分类结果是否准确。

\textbf{问题二：}如果附件1中123家企业的信用等级情况的分类结果准确，请对附件2中的302家无信贷记录企业的信用等级情况进行分类。如果不准确，请按照你们的分类标准对附件2中的302家无信贷记录企业的信用等级情况进行分类。
...
问题一要求验证银行对123家企业的信用评级是否准确。本题面临的核心困难在于：不存在绝对正确的真实标签作为评判基准，附件中给出的银行评级本身就是待检验对象。因此，我们必须找到一种不依赖于正确标签的判别方式，我们可以通过间接验证方式来确定银行评级的正确性和科学性，如果银行评级是客观准确的，那么利用企业交易数据中的客观特征来构建的模型应当能够较好地反映银行给出的分级；反之，若通过这些客观特征构建的模型预测的分级与银行的分级标签大量不一致，则说明银行评级存在偏差，并不准确。

基于这一验证逻辑，我们发现解题需要解决三个方面的问题：

第一是特征构建问题。附件中为约37万条发票中的金额、时间、类别等数据，我们需把单个企业的所有发票数据聚合为能代表该企业的特征，以此来构建合适的模型来解决问题。

第二是降低特征维度问题。直接从发票数据中获得的19个基础变量间存在强相关性，直接以此来构建模型会导致模型难以判断评级差异究竟由哪个变量引起，所以我们需要合适的降维方法来将特征维度降低，降低各个特征之间的相关性，使各个特征对评级的贡献更易被区分。

第三是验证与评估问题。验证模型需要满足两个条件：一是能够输出各类别的预测概率，以便量化模型对每个等级的置信程度；二是模型参数应具有可解释性，使得我们能够从系数方向判断究竟是哪些特征在驱动评级分歧。此外，仅凭单一模型的准确率难以区分是特征本身区分力不足还是模型的选择不当，因此有必要引入非线性模型作为对照，若两者表现均有限，则瓶颈在数据而非方法。评估体系上，需要混淆矩阵与一致性系数综合评判分类性能。此外，原始数据横跨2017至2020年共三年，企业经营状况动态变化，需通过时效性稳健性检验，考察时间的不同是否影响结论。

\subsection{问题二的分析}

问题二如何求解取决于问题一的结论。题目明确指出：若附件1中123家企业的分类结果准确，则直接对302家企业进行分类；若不准确，则需按照我们自己的分类标准进行分类。因此需要分两种情形讨论。

\textbf{情形一：银行评级准确。}若问题一的验证表明银行分级科学且准确，则说明银行评级体系本身可靠，可直接以原标签为训练目标，利用问题一建立的分类模型对302家企业进行有监督预测。

\textbf{情形二：银行评级不准确。}若问题一验证表明银行分类标准不太准确和科学，则不能简单沿用原标签进行模型的训练——否则模型只会复现银行的偏差。此时需要建立一套独立于银行主观判断的客观分类标准。而新标准必须满足两个条件：一是客观性，分类依据应完全由数据驱动而非主观判断；二是泛化能力，标准不能仅对123家有效，还需推广到302家新企业。

针对情形二，需要依次解决以下三个方面的问题：

第一，确定各个特征的相对重要性。信用评级涉及规模、活跃度、风险、趋势、集中度等多个维度，权重不应由主观判断决定，需要一种纯粹依据数据信息差异程度的客观赋权方法。

第二，建立新的的分级标准并确定新标签。既然银行标签不太准确，就需要从原123家公司的数据中重新确定各家企业分级。

第三，将模型泛化到302家新企业。分类标准应只能从原123家企业数据中确定，模型也应该从原123家企业中的数据确定。因此需要训练一个有监督分类器，学习"特征→新标签"的映射规则，再将该规则应用到302家新企业的特征上进行分级。
...
为弥补传统财务指标无法刻画供应链风险与发展趋势的局限，本文构造了以下特征。
...
\end{equation}
其中$N_{in}^k$表示来自供应商$k$的发票数量。集中度过高意味着企业过度依赖少数交易对手，抗风险能力弱；集中度过低则表明企业缺乏稳定的上下游合作关系，交易模式不可预期，经营的可持续性难以判断。

为分析企业的发展趋势，定义平均营收增长率$F_{10}$：
\begin{equation}
F_{10} = \frac{1}{T-1}\sum_{t=1}^{T-1}\frac{R_{t+1} - R_t}{|R_t| + \epsilon}
\label{eq:growth_rate}
\end{equation}
其中$R_t$为第$t$年的销项有效价税合计，$\epsilon$为防止除零的平滑项。
计算后对$F_{10}$截断至$[-1, 5]$剔除极端值，截断后均值1.28，持续增长的企业信用风险较低，持续萎缩的企业则相反。

稳定度与增长率不参与PCA降维：它们本身为无量纲比值，与19个发票汇总变量差别较大，若混入PCA会干扰得到的因子的经济含义解释。因此我们将两类特征并列，PCA仅对发票汇总变量降维，稳定度和增长率作为独立特征与PCA因子并列进入模型。

\subsubsection{特征降维与特征体系构建}
19个基础变量间存在强相关性，直接构造模型会导致回归系数不稳定，本文采用PCA主成分降维。在进行PCA之前，需验证19个基础变量之间是否存在足够的相关性以确保降维有意义。KMO检验值为0.722，大于0.5的适用阈值，表明变量间的偏相关系数相对于简单相关系数较小。Bartlett球形检验$\chi^2=10440$，$p<0.001$，确认变量间并非彼此独立，所以我们采用PCA主成分降维。
...
降维前19个变量各自携带的信息量，在降维后有86.22%被这4个公因子所涵盖，仅损失不到14%的信息，说明4个因子已足以代表原始变量的主要特征。特征值在第4个因子后急剧下降趋于平缓，印证了取4个因子的合理性。
...
最终输入模型的特征向量为$\mathbf{x} = [F_1, F_2, F_3, F_4, F_7, F_8, F_9, F_{10}]^T$，共8维，兼顾了交易规模、活跃度、风险状况与发展趋势。
...
同时引入XGBoost作为辅助验证模型\cite{ref3}，采用5折分层交叉验证：将123家企业按各等级比例均衡地分为5份，每次以4份训练、1份测试，轮换5次后取平均准确率。分层保证每折中各等级占比与总体一致，避免少数类在某一折中缺失导致评估波动。该交叉验证中得到的准确率可作为特征体系分类上限的参照，得到的特征重要性排序也可佐证各因子的相对贡献。

\subsubsection{模型求解}

\step{1} 将123家企业的8维特征向量$\mathbf{x}$与银行标签$y$组成训练集，对多元Logit模型进行最大似然估计，获得各特征对B/C/D三个等级相对于A级的回归系数。

\step{2} 以相同特征和标签对XGBoost进行5折分层交叉验证。

\step{3} 基于Logit预测结果，计算总体准确率、Cohen's Kappa一致性系数及各级别召回率，输出混淆矩阵。

\step{4} 仅使用2019--2020年近期数据重新提取特征并预测，对比全数据与近期数据的准确率变化，检验评估结论对数据窗口的敏感程度。

\subsubsection{结果展示}

首先检验模型整体有效性。LLR检验$p<0.001$，表明8个特征对评级分类具有显著的解释力；$R^2$为0.238，说明模型能解释约24\%的评级变化。

其次分析各特征的驱动方向。回归系数显示，活力因子$F_1$和金额因子$F_2$的系数显著为负，表明交易活跃度和资金规模是将企业维持在高等级的核心支撑；进销稳定度$F_9$（$\beta=30.62$）显著为正，表明供应链集中度越高，企业越被推向低等级。

最后评估模型预测与银行标签的一致程度。总体准确率54.47\%，Cohen's Kappa为0.384：
\begin{equation}
\kappa = \frac{p_o - p_e}{1 - p_e}
\label{eq:kappa}
\end{equation}
其中$p_o$为观测一致率即总体准确率，$p_e$为期望随机一致率。按解释标准，$\kappa<0.4$为一致性一般，$0.4\sim0.6$为中等，$0.6\sim0.8$为较好。本文$\kappa=0.384$处于"一般"水平，表明模型仅能部分复现银行评级逻辑，两者之间存在显著分歧。
...
如图\ref{fig:xgb_cm}所示，XGBoost五折交叉验证准确率为43.09\%，反而低于Logit的54.47\%。XGBoost的拟合能力更强，若特征中存在Logit捕捉不到的非线性分类信息，XGBoost理应表现更优。然而事实相反，这说明123个样本的信息量不足以支撑复杂模型学到稳定规律，分类的瓶颈在于特征本身的区分力有限，而非模型选择问题。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.5\textwidth]{fig6_xgb_importance.png}
...
\textbf{时效性稳健性检验：}仅使用2019--2020年近期数据重新预测，结果如图\ref{fig:robust}所示，准确率骤降至30.89\%、Kappa 0.133，相比全数据54.47\%下降23.6个百分点。原因在于短期截断数据无法计算跨年增长率$F_{10}$，证实了信用评估必须依赖长周期的交易数据这个观点。
...
鉴于问题一证实了银行分级标准不够准确，本文采用熵权法确定权重，再进行加权K-Means聚类，最终利用新的标签训练模型进行有监督预测的递进策略。
...
\textbf{阶段三：有监督分类预测。}以重构标签$Y'$为目标，训练分类器将聚类规则泛化至302家无信贷记录企业，输出预测结果。

\subsubsection{模型求解}

\step{1} 对8个特征按方向做min-max标准化后，计算各特征的信息熵$e_j$、差异系数$d_j$与客观权重$w_j$。

\step{2} 对标准化特征做加权变换$z'_{ij}=\sqrt{w_j}\cdot z_{ij}$，在变换后的空间$\mathbf{Z}'$中执行K-Means聚类（$k=4$）。聚类完成后，按各簇中心的加权综合得分降序映射为A'/B'/C'/D'四档，生成123家企业的新标签$Y'$。

\step{3} 以新标签$Y'$为目标，训练Logit与XGBoost两个分类器，并分别使用8维PCA特征和23维原始特征两组输入，通过五折交叉验证选取最优组合。

\step{4} 用分类器对302家无信贷记录企业进行评级预测，输出各企业的类别概率与最终评级，并计算各等级企业在8个特征上的均值.
\subsubsection{结果展示}
...
新标签分布为A'/B'/C'/D'=9/25/50/39，分布均衡。各等级特征均值如图\ref{fig:cluster_profile}所示：A'为极高活力与金额的头部优质企业；B'为增长率极高但规模偏低的高速成长型企业，$F_{10}=4.69$；C'为各因子接近均值的普通中型企业；D'为低活跃、高依赖度的低信用企业，$F_8=0.69$。

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{fig8_cluster_profile.png}
\caption{加权K-Means各等级聚类中心特征均值}
\label{fig:cluster_profile}
\end{figure}

如图\ref{fig:label_cmp}所示，新标签与银行标签的调整兰德指数ARI仅0.039，一致性极低，揭示了新分级标准同银行原标准之间的分歧，一定程度上反映了新分级标准的准确性。
...
特征重要性排序如图\ref{fig:p2_imp}所示。一个值得关注的现象是：稳定度$F_8/F_9$在熵权法中权重极低（合计仅11.1\%），但在分类器中重要性却排第2、3位。两者并不矛盾：熵权法衡量的是特征在所有企业间的线性离散程度，稳定度在多数企业间取值接近，故权重低；而分类器能够捕捉到，当集中度超过某一临界值后企业信用显著下降。多数企业未触及该阈值时稳定度的区分作用不显著，少数触及阈值的企业则被强烈区分，这种非线性阈值效应是线性赋权方法无法反映的。
...
预测置信度分布如图\ref{fig:confidence}所示：均值0.882，中位数0.962，高置信企业即置信度大于0.7的企业共253家，占83.8\%。
...
\textbf{标签重构合理性：}新标签$Y'$分布均衡。且ARI仅0.039揭示客观标准与银行主观判断之间的结构性分歧，印证了问题一B/C中间等级存在严重混杂的结论。

\textbf{分类预测可靠性：}91\%的准确率远高于问题一的54.5\%，正确表述为：客观重构标签的内部一致性强，说明熵权聚类标准具有清晰的特征到等级映射关系，分类器能够有效泛化这一映射。302家画像与123家聚类中心高度一致，预测置信度均值0.882，可靠性高。
...
本文深挖发票流水背后的供应链网络拓扑结构，构建了更具区分力的企业信用画像。针对传统降维方法导致的标签极端偏斜问题，提出在多维特征空间直接执行加权聚类，在保留多维结构的同时实现了标签的均衡分布。研究揭示了信用风险并非线性累积而是存在临界阈值效应，为银行差异化风控提供了理论支撑。
...
problem2\_extra\_plots.py   & 问题二额外可视化代码 \\
\midrule
\multicolumn{2}{l}{\textbf{结果文件}} \\
logit\_confusion\_matrix.csv   & 问题一混淆矩阵 \\
logit\_summary.txt   & 问题一回归系数表 \\
entropy\_weights.csv   & 问题二熵权法权重表 \\
cluster\_centers\_p2.csv   & 问题二聚类中心及映射评级 \\
new\_labels\_123.csv   & 问题二123家企业新标签 \\
p2\_classifier\_cv.csv   & 问题二分类器交叉验证对比 \\
p2\_prediction\_302.csv   & 问题二302家企业预测结果 \\
\midrule
