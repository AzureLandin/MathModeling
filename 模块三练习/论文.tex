...
同时引入XGBoost作为辅助验证模型，采用5折分层交叉验证：其一，准确率可作为特征体系分类上限的参照；其二，特征重要性排序可佐证各因子的相对贡献。
...
\bibitem{ref1} 赵星然, 等. 基于小微企业发票数据的信用风险评价模型研究[J]. 统计与决策, 2022, 第38卷第12期.

\bibitem{ref2} 姜明辉. 商业银行信用风险评估模型与实证分析[M]. 科学出版社, 2018.
...
文件名 & 类型 & 说明 \\
\midrule
problem1\_preprocess.py & Python & 问题一数据清洗与特征提取代码 \\
problem1\_logit.py & Python & 问题一多元逻辑回归求解代码 \\
problem2\_entropy\_cluster.py & Python & 问题二熵权法与加权聚类代码 \\
problem2\_classifier.py & Python & 问题二有监督分类与预测代码 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{lstlisting}[language=Python, caption={核心特征工程与熵权法计算代码片段}]
import pandas as pd
import numpy as np
from sklearn.preprocessing import MinMaxScaler

def calculate_entropy_weights(data, directions):
    scaler = MinMaxScaler()
