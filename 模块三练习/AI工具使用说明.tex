...
\begin{enumerate}[label=(\arabic*)]
  \item 文献搜集环节：生成小微企业信用评级的中英文检索关键词，梳理PCA降维、多元Logit回归、熵权法、加权K-Means聚类等方向的文献观点，搭建综述写作框架。
  \item 模型推导环节：整理PCA因子分析适用性检验与Varimax旋转的数学依据、多元Logit回归系数经济含义解释、熵权法客观赋权公式推导、加权欧氏距离等价变换逻辑，对比各阶段算法优劣。
  \item 代码编写环节：生成数据清洗与Winsorize缩尾处理、PCA因子分析、多元Logit回归、XGBoost交叉验证、熵权法赋权、加权K-Means聚类、有监督分类预测的Python代码骨架，排查索引越界与聚类收敛等报错，精简冗余代码。
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item 正文润色环节：优化口语化表述，统一主成分分析、多元Logit回归、熵权法、加权K-Means等专业术语，结合仿真数据产出图表描述与结果分析初稿。
...
\textbf{Query1：}帮我们找小微企业信用评级方面的文献，中英文检索词都给一下，再说下综述怎么写。

\textbf{Output：}关键词：信用评级、PCA降维、多元Logit回归、熵权法、加权K-Means；综述思路：从特征工程到评级验证再到客观标准重构递进展开。

\textbf{Query2：}推导一下PCA因子分析适用性检验与Varimax旋转的依据，再推导熵权法公式和加权K-Means等价变换，对比各算法优劣。

\textbf{Output：}KMO与Bartlett检验推导、PCA旋转后因子载荷经济含义、熵权法信息熵与差异系数数学表述、加权欧氏距离等价变换步骤；表格对比各算法优劣与局限。

\textbf{Query3：}写个Python代码框架，包括数据清洗与Winsorize处理、PCA因子分析、多元Logit回归、XGBoost交叉验证、熵权法赋权、加权K-Means聚类、有监督分类预测，按模块拆分加注释。

\textbf{Output：}分模块代码骨架：数据预处理、特征工程、评级验证、客观赋权、聚类重构、分类预测；列举索引越界与聚类收敛的排查方案。
...
\textbf{Query2：}采纳PCA与Varimax旋转推导步骤与算法优劣对比框架；人工补充供应链稳定度与平均营收增长率等专属条件，代入123家企业坐标参数验算全部公式，修正变量定义偏差，自主确定组合求解策略。

\textbf{Query3：}采纳分模块代码框架；人工适配123家与302家企业坐标改写代码，逐行调试修复索引越界与聚类不收敛bug，调整Logit与XGBoost超参数，新增供应链特征构造与置信度计算自定义函数，独立运行仿真导出全部配图。
