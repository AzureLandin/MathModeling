...
\centering
\vspace*{2cm}
{\zihao{2}\heiti\bfseries 全国大学生数学建模竞赛论文\par}
\vspace{1.5cm}
{\zihao{-2}\heiti\bfseries 论文标题\par}
\vspace{2cm}
{\zihao{4}\songti
\begin{tabular}{rl}
参赛队号：& \underline{\hspace{4cm}} \\[0.8em]
参赛队员：& \underline{\hspace{4cm}} \\[0.8em]
& \underline{\hspace{4cm}} \\[0.8em]
& \underline{\hspace{4cm}} \\[0.8em]
指导教师：& \underline{\hspace{4cm}} \\[0.8em]
\end{tabular}\par}
\vfill
{\zihao{4}\songti \the\year 年 \the\month 月\par}
\end{titlepage}
...
% 写作框架：
% (1) 先用一句话概括已知条件（给了什么数据/参数）；
% (2) 再逐条列出各问题，每条包含：问题类型 + 需要做什么 + 目标是什么；
% (3) 可简要说明各问题之间的递进关系（从简单到复杂）。
%
% 示例写法：
% 现已知道……的初始参数和……数据，本文需要解决以下问题：
%
% 问题一：在给定参数下，建立……模型，计算……（基础计算问题）。
%
% 问题二：在问题一的基础上，将……作为决策变量，以……为目标，
% 建立优化模型，求解最优策略（单因素优化）。
%
% 问题三：将问题二拓展至……情形，需要协调……（多因素协同）。
%
% 问题四：进一步考虑……情形，核心难点在于……（更复杂约束）。
%
% 问题五：综合以上所有情形，需要同时解决……和……问题，
% 实现全局最优（最复杂情形）。
