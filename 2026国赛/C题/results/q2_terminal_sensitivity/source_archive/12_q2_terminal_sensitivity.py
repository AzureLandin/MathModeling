...
SNAPSHOT_FILES = ['code/08_q2_quantile_experiment.py', 'code/05_q2_baseline.py',
                  'code/02_q1_baseline.py']


def fingerprint():
    """Hashes of the inputs and sources that define this experiment, plus its signature."""
    hashes = {rel: digest(ROOT / rel) for rel in INPUTS}
    snapshot_hashes = {rel: digest(SNAP / rel) for rel in SNAPSHOT_FILES}
    signature = hashlib.sha256(json.dumps(
        dict(candidates=list(CANDIDATES), policies=POLICIES,
             input_sha256=hashes, snapshot_sha256=snapshot_hashes),
        sort_keys=True).encode()).hexdigest()
    return hashes, snapshot_hashes, signature


def stage_register(amend_reason=None):
    OUT.mkdir(parents=True, exist_ok=True)
    hashes, snapshot_hashes, signature = fingerprint()
    ...
    plan_path = OUT / 'experiment_plan.md'
    existing = json.loads((OUT / 'registration.json').read_text(encoding='utf-8')) \
        if (OUT / 'registration.json').exists() else None
    if existing is not None:
        assert amend_reason, ('a registration already exists; pass --amend-reason to record a '
                             'documented amendment instead of overwriting it')
        assert existing['candidates_kWh'] == list(CANDIDATES), 'the frozen grid must not change'
        assert existing['policies'] == POLICIES, 'the frozen policies must not change'
        assert existing['fixed_conditions'] == record['fixed_conditions'], \
            'the fixed conditions must not change'
        amendments = existing.get('amendments', [])
        amendments.append(dict(
            ...
            reason=amend_reason,
            candidate_grid_unchanged=True, fixed_conditions_unchanged=True,
        ...
        with plan_path.open('a', encoding='utf-8') as handle:
            ...
            for index, item in enumerate(amendments, start=1):
                handle.write(
                    f"{index}. {item['amended_utc']}：{item['reason']}\n\n"
                    f"   原签名 `{item['previous_signature'][:32]}` → 新签名 "
                    f"`{item['new_signature'][:32]}`。候选网格 4800/6000/7200 未变，"
                    f"固定条件未变，三组策略定义未变；修订只影响运行后的检验代码。\n\n")
    else:
        plan_path.write_text(PLAN_TEXT, encoding='utf-8')
    save(OUT / 'registration.json', record)
    print(f'registered: candidates={list(CANDIDATES)} signature={signature[:16]}'
          + (f' (amended, {len(record.get("amendments", []))} amendment(s))'
             if record.get('amendments') else ''), flush=True)
...
def assert_registered_by_source():
    """For --stage run: the registration must exist and match the sources on disk now."""
    path = OUT / 'registration.json'
    assert path.exists(), 'run --stage register first'
    saved = json.loads(path.read_text(encoding='utf-8'))
    _, _, signature = fingerprint()
    assert saved['signature'] == signature, \
        'the registered source/input hashes differ from the current files'
    return saved
...
def independent(name, target):
    ...
    pv = d.pv_kW.to_numpy()
    nominal = d.groupby('date', sort=False).nominal_state_end_kWh.last().to_numpy()
    day_end = d.groupby('date', sort=False).state_end_kWh.last().to_numpy()
    fee = p * q + 5 * p * e
    ...
    morning = d.slot < 60
    day_start = d.groupby('date', sort=False).state_start_kWh.first().to_numpy()
    row = dict(
        ...
        full_end_days=int((day_end >= 10800 - 1e-6).sum()),
        empty_slots=int((s1 <= 1200 + 1e-6).sum()),
        mean_initial_kWh=float(day_start.mean()),
        mean_slot_state_kWh=float(s0.mean()),
        initial_kWh=float(s0[0]), final_kWh=float(s1[-1]),
        segments=int(len(d)), days=int(d.date.nunique()),
    )
    return row, physical, accounting


def causal_checks(load, pv, price, warm):
    """Future data must not change today's 00:00 forecast, correction or plan.

    The initial state only has to be identical between the base and the perturbed run; the
    executed state of the 6000 kWh group is used so the check runs at a real operating point.
    """
    m = module()
    recorded = pd.read_csv(OUT / 'T_q80_6000' / 'daily_summary.csv')
    state_by_day = {int((pd.Timestamp(r.date) - m.bm.DATES[0]).days): float(r.initial_kWh)
                    for r in recorded.itertuples()}
    state_by_day.update({int(k): float(v) for k, v in warm.items()})
    rows = []
    for target in CANDIDATES:
        for k in (31, 200):
            ...
            row = dict(target_kWh=target, day_index=k,
                       day=str(m.bm.DATES[k].date()), initial_state_kWh=initial,
...
def monthly_tables():
    ...
    for p in POLICIES:
        ...
        totals[p['id']] = monthly
        monthly.assign(strategy_id=p['id']).to_csv(
            OUT / p['id'] / 'monthly_totals.csv', index=False, encoding='utf-8-sig')
    base = totals['T_q80_6000'].set_index('month')
    for sid, monthly in totals.items():
        frame = monthly.set_index('month')
        frame = frame.assign(
            delta_total_vs_6000_yuan=frame.total_cost_yuan - base.total_cost_yuan,
            delta_emergency_vs_6000_yuan=frame.emergency_cost_yuan - base.emergency_cost_yuan,
            delta_unused_vs_6000_kWh=frame.unused_kWh - base.unused_kWh).reset_index()
...
def main():
    ...
    parser.add_argument('--amend-reason', default=None,
                        help='documented reason when a registration already exists')
    args = parser.parse_args()
    assert Path(sys.prefix).name == 'math_modeling', sys.prefix
    np.random.seed(20260910)

    if args.stage == 'register':
        stage_register(args.amend_reason)
        return
    if args.stage == 'all':
        registration = stage_register(args.amend_reason)
    else:
        registration = assert_registered_by_source()
    ...
    independent_rows, physical_by_group, accounting_by_group = [], {}, {}
    for policy in POLICIES:
        row, physical, accounting = independent(policy['id'], policy['terminal'])
        independent_rows.append(row)
        physical_by_group[policy['id']] = physical
        accounting_by_group[policy['id']] = accounting
    ...
    checks = dict(
        max_physical_violation_kWh=max(max(physical_by_group[sid][k] for k in
                                          ['balance_kWh', 'state_kWh', 'bounds_kWh', 'power_kWh',
                                           'nonnegative_kWh', 'mutex_kWh'])
                                      for sid in physical_by_group),
        max_day_continuity_kWh=max(v['continuity_kWh'] for v in physical_by_group.values()),
        max_nominal_terminal_error_kWh=max(v['nominal_terminal_kWh']
                                           for v in physical_by_group.values()),
        max_initial_offset_kWh=max(v['initial_state_kWh'] for v in physical_by_group.values()),
        max_accounting_error=max(max(v.values()) for v in accounting_by_group.values()),
        max_energy_identity_kWh=max(v['energy_identity_kWh']
                                    for v in accounting_by_group.values()),
    ...
    save(OUT / 'validation.json', dict(
        ...
        per_group_physical=physical_by_group,
        per_group_accounting=accounting_by_group,
        future_perturbation=perturbation, shared_protection=shared))

    figure_names = figures(summary, monthly, hourly, extremes)
    attachments_now = {rel: digest(ROOT / rel)
                       for rel in ['附件/附件1.xlsx', '附件/附件2.xlsx']}
    attachments_unchanged = all(attachments_now[rel] == registration['input_sha256'][rel]
                                for rel in attachments_now)
    assert attachments_unchanged, 'an attachment changed during this run'
    save(OUT / 'run_manifest.json', dict(
        ...
        attachment_sha256_now=attachments_now, attachments_unchanged=attachments_unchanged,
