...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=OUT, help="导出目录，默认提交包/result")
    ...
    missing_sources = [source for _, source, _, _ in selected if not source.exists()]
    if missing_sources:
        packaged = [OUT / filename for filename, _, _, _ in selected]
        missing_outputs = [path for path in packaged if not path.exists()]
        if missing_outputs:
            raise FileNotFoundError(
                "缺少原始冻结账本且提交包内没有对应正式结果："
                + ", ".join(str(path) for path in missing_outputs)
            )
        print(json.dumps({
            "status": "verified_existing",
            "message": "提交包已脱离原项目，保留并核验现成正式 CSV",
            "files": [{"文件名": path.name, "SHA256": _sha256(path)} for path in packaged],
