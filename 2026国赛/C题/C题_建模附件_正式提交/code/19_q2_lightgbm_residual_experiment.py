...
Specification: reports/问题二/实验方案/问题二_LightGBM残差预测实验方案.md (single-run task book).
...
Five groups, one fixed LightGBM configuration, one shared model per target per day:
    G0_naive_gate      naive load + naive PV, both under the unified PV gate (matched baseline)
    G1_ridge_gate      legacy ridge archive + gate; reproduces module-16 G3_joint_gate
    G2_lightgbm_gate   legacy LightGBM archive + gate; the previously signed run
    G3_ridge_fixed     ridge archive with the pre-feature naive fallback repaired
    G4_lightgbm_fixed  LightGBM archive with that fallback repaired (primary treatment)

Module 14 left the days before the feature-valid day unpublished (NaN), which silently corrupted
the early-February q80 order statistic. G1/G2 keep that behaviour so the signed run stays
reproducible; G3/G4 publish the naive fallback required by the plan and form the primary,
matched comparison G4 - G3.
"""
...
PLAN_MD = ROOT / 'reports/问题二/实验方案/问题二_LightGBM残差预测实验方案.md'
REPORT_MD = ROOT / 'reports/问题二/实验结果/问题二_LightGBM残差预测实验结果报告.md'
...
REFERENCE_G2_LEGACY_YUAN = 13680001.420638
REFERENCE_ENERGY_TOL_KWH = 1e-6
REFERENCE_COST_TOL_YUAN = 1e-4
TIE_RELATIVE = 1e-10

# reports/问题二/实验方案/问题二_LightGBM残差预测实验方案.md section 3.2 -- one fixed configuration, no search.
LGBM_PARAMS = dict(
...
PARAMETERS = dict(
    ...
    groups=['G0_naive_gate', 'G1_ridge_gate', 'G2_lightgbm_gate', 'G3_ridge_fixed',
            'G4_lightgbm_fixed'],
    primary_comparison='G4_lightgbm_fixed minus G3_ridge_fixed (matched, gap-repaired pair)',
    legacy_comparison='G2_lightgbm_gate minus G1_ridge_gate (previously signed pair)',
    fallback_rule='days before the feature-valid day publish the causal naive forecast and their '
                  'realised errors enter q80 (module 14 left them unpublished/NaN)',
)
...
GROUPS = [
    ...
    dict(id='G1_ridge_gate', load='ridge', pv='ridge',
         role='legacy ridge archive + gate; reproduces module-16 G3_joint_gate (has the '
              'pre-feature publication gap inherited from module 14)'),
    dict(id='G2_lightgbm_gate', load='lightgbm', pv='lightgbm',
         role='legacy LightGBM archive + gate; the previously signed run, kept for continuity'),
    dict(id='G3_ridge_fixed', load='ridge', pv='ridge',
         role='ridge archive with the pre-feature days repaired to the naive fallback'),
    dict(id='G4_lightgbm_fixed', load='lightgbm', pv='lightgbm',
         role='LightGBM archive with the pre-feature days repaired to the naive fallback '
              '(primary treatment)'),
]
PRIMARY_TREATMENT = 'G4_lightgbm_fixed'
PRIMARY_BASELINE = 'G3_ridge_fixed'
LEGACY_TREATMENT = 'G2_lightgbm_gate'
LEGACY_BASELINE = 'G1_ridge_gate'
...
LIVE_INPUTS = SNAPSHOT_INPUTS + [
    ...
    'reports/问题二/实验方案/问题二_LightGBM残差预测实验方案.md',
...
OWN_NEW_REL = {'code/19_q2_lightgbm_residual_experiment.py',
               'reports/问题二/实验结果/问题二_LightGBM残差预测实验结果报告.md'}
...
def build_lightgbm_forecasts(load, pv, dates, upto=None, save_models=False, verbose=False):
    ...
    # Before the feature-valid day the plan still requires a published forecast: the naive
    # fallback. Its realised error is a valid issued error and must enter q80. Module 14 left
    # these days as NaN, which silently corrupted the early-February order statistic.
    for char in ('load', 'pv'):
        threshold = spec_map[char]['threshold']
        issued[char][1:threshold] = naive[char][1:threshold]
        for k in range(1, threshold):
            fit_log.append(dict(target=char, day_index=k, date=str(dates[k].date()),
                                n_train_days=0, n_rows=0, trained=False,
                                branch='feature_history_fallback', n_trees=0,
                                model_file='', model_sha256='', elapsed_seconds=0.0,
                                reason=f'day_index<{threshold}: published naive fallback'))
...
def reintroduce_gap(values, threshold):
    """Recreate the module-14 behaviour that left pre-feature days unpublished (NaN)."""
    out = np.array(values, dtype=float, copy=True)
    out[1:threshold] = np.nan
    return out


def repair_archive_gap(values, naive_values, threshold):
    """Fill the pre-feature days of an inherited archive with the causal naive fallback."""
    out = np.array(values, dtype=float, copy=True)
    out[1:threshold] = np.asarray(naive_values, dtype=float)[1:threshold]
    return out
...
def bundles_for(lightgbm, ridge, load, pv, dates, mask):
    ...
    G1/G2 keep the inherited module-14 gap (pre-feature days left unpublished) so the previously
    signed run stays reproducible; G3/G4 repair that gap with the naive fallback required by the
    plan, and are the matched pair used for the primary comparison.
    """
    m = parent()
    th = {c: m.TARGET_SPEC[c]['threshold'] for c in ('load', 'pv')}
    naive = lightgbm['naive']
    lgbm = lightgbm['issued']
    gated_naive_pv = apply_gate(naive['pv'], mask)

    def bundle(load_values, pv_values):
        return dict(naive=dict(load=naive['load'], pv=gated_naive_pv),
                    issued=dict(load=load_values, pv=apply_gate(pv_values, mask)))

    legacy_lgbm = dict(load=reintroduce_gap(lgbm['load'], th['load']),
                       pv=reintroduce_gap(lgbm['pv'], th['pv']))
    fixed_ridge = dict(load=repair_archive_gap(ridge['load'], naive['load'], th['load']),
                       pv=repair_archive_gap(ridge['pv'], naive['pv'], th['pv']))
    fixed_lgbm = dict(load=repair_archive_gap(lgbm['load'], naive['load'], th['load']),
                      pv=repair_archive_gap(lgbm['pv'], naive['pv'], th['pv']))
    return {
        'G0_naive_gate': bundle(naive['load'], naive['pv']),
        'G1_ridge_gate': bundle(ridge['load'], ridge['pv']),
        'G2_lightgbm_gate': bundle(legacy_lgbm['load'], legacy_lgbm['pv']),
        'G3_ridge_fixed': bundle(fixed_ridge['load'], fixed_ridge['pv']),
        'G4_lightgbm_fixed': bundle(fixed_lgbm['load'], fixed_lgbm['pv']),
    }
...
# --------------------------------------------------------------------------------------
def perturbation_checks(load, pv, price, dates, lightgbm, bundles, warm_states, clean_result):
    """Six future-truth perturbations. For the issue day the published forecast, the training
    rows, the q80 protection and the ordinary plan must all stay unchanged."""
    m = parent()
    group = next(g for g in GROUPS if g['id'] == PRIMARY_TREATMENT)
    clean_archive = clean_result['archive']
    rows = []
    for k in (31, 171, 354):
        for kind in ('load', 'pv'):
            ...
            rebuilt_bundles = bundles_for(rebuilt, load_ridge_issued(), ll, vv, dates,
                                          support_mask(vv))
            forecast_error = max(
                float(np.max(np.abs(rebuilt_bundles[PRIMARY_TREATMENT]['issued'][t][k]
                                    - bundles[PRIMARY_TREATMENT]['issued'][t][k])))
                for t in ('load', 'pv'))
            clean_rows = {e['target']: e['n_rows'] for e in lightgbm['fit_log']
                          if e['day_index'] == k and e['trained']}
            rebuilt_rows = {e['target']: e['n_rows'] for e in rebuilt['fit_log']
                            if e['day_index'] == k and e['trained']}
            training_match = bool(clean_rows == rebuilt_rows and clean_rows)
            archive = m.make_group_archive(group, ll, vv, dates, rebuilt_bundles[PRIMARY_TREATMENT])
            state = float(clean_result['daily'].iloc[k - WARMUP_DAYS].initial_kWh)
            q80_error = float(np.max(np.abs(
                archive.adjustment(k, RESIDUAL_WINDOW, THETA)[0]
                - clean_archive.adjustment(k, RESIDUAL_WINDOW, THETA)[0])))
            planned = clean_result['dispatch']
            planned = planned[planned.date == str(dates[k].date())].planned_kWh.to_numpy()
            plan_error = float(np.max(np.abs(
                archive.plan_day(k, THETA, RESIDUAL_WINDOW, price, state)[0] - planned)))
            passed = bool(forecast_error < 1e-6 and training_match
                          and q80_error < 1e-6 and plan_error < 1e-6)
            assert passed, (k, kind, forecast_error, training_match, q80_error, plan_error)
            rows.append(dict(day=k, kind=kind, forecast_max_kW=forecast_error,
                             training_rows_match=training_match, q80_max_kWh=q80_error,
                             plan_max_kWh=plan_error, passed=passed))
    return dict(cases=rows, all_passed=bool(all(r['passed'] for r in rows)),
                scope='published forecast + training rows + q80 protection + ordinary plan '
                      'on the issue day')


def feedback_prefix_check(load, pv, price, dates, lightgbm, bundles, clean_result):
    ...
    return dict(day=k, morning_state_max_kWh=morning_state, morning_plan_max_kWh=morning_plan,
                scope='morning plan and executed state prefix (slots 0-71)',
...
def boundary_checks(load, pv, dates, lightgbm, mask):
    ...
    thresholds = {c: m.TARGET_SPEC[c]['threshold'] for c in ('load', 'pv')}
    fallback_ok = True
    for char in ('load', 'pv'):
        th = thresholds[char]
        fallback_ok = fallback_ok and bool(
            np.allclose(lightgbm['issued'][char][1:th], lightgbm['naive'][char][1:th]))
        fallback_ok = fallback_ok and bool(np.isnan(lightgbm['issued'][char][0]).all())
    out['pre_feature_fallback'] = dict(
        thresholds=thresholds,
        note='before the feature-valid day the published forecast is the causal naive fallback and '
             'day index 0 (2025-01-01) has no published prediction at all; module 14 left the '
             'whole pre-feature block unpublished',
        passed=bool(fallback_ok))

    log = pd.DataFrame(lightgbm['fit_log'])
    fitted = log[log.branch == 'fitted']
    constant = log[log.branch == 'constant_target']
    out['branch_consistency'] = dict(
        fitted_days=int(len(fitted)), fitted_with_zero_trees=int((fitted.n_trees == 0).sum()),
        constant_days=int(len(constant)), constant_with_trees=int((constant.n_trees > 0).sum()),
        fallback_days=int((log.branch == 'feature_history_fallback').sum()),
        insufficient_days=int((log.branch == 'insufficient_history').sum()),
        note='a fitted day must carry at least one tree; a constant-target day must carry none',
        passed=bool(len(fitted) and (fitted.n_trees == 0).sum() == 0
                    and (constant.n_trees > 0).sum() == 0))

    probe = fitted.head(1)
    if len(probe):
        entry = probe.iloc[0]
        char = entry.target
        k = int(entry.day_index)
        spec = m.TARGET_SPEC[char]
        actual = load if char == 'load' else pv
        # Load from the saved text rather than the path: LightGBM's C++ file API cannot open
        # non-ASCII paths on Windows (this project lives under a Chinese-named directory).
        saved_text = (OUT / entry.model_file).read_text(encoding='utf-8')
        booster = lgb.Booster(model_str=saved_text)
        rebuilt = np.maximum(0.0, lightgbm['naive'][char][k]
                             + np.asarray(booster.predict(spec['feat_fn'](actual, dates, k)),
                                          dtype=float))
        difference = float(np.max(np.abs(rebuilt - lightgbm['issued'][char][k])))
        out['model_save_reload'] = dict(target=char, day_index=k, max_difference_kW=difference,
                                        model_file=entry.model_file, model_sha256=entry.model_sha256,
                                        note='the saved model text is reloaded and must reproduce '
                                             'the issued forecast for that day',
...
def contrast(comparison, treatment, baseline):
    """One-row contrast of `treatment` against `baseline`, recomputed from the raw totals.

    The frozen comparison table differences every group against results[0]; the plan's primary
    and legacy pairs each need their own baseline.
    """
    row = comparison[comparison.strategy_id == treatment].copy()
    base = comparison[comparison.strategy_id == baseline].iloc[0]
    ...
    for column, source in (('delta_planned_cost_yuan', 'planned_cost_yuan'),
                           ...
                           ('delta_unused_kWh', 'unused_kWh'),
                           ('delta_loss_kWh', 'loss_kWh')):
        row[column] = row[source] - base[source]
    row['delta_total_pct'] = 100 * row.delta_total_cost_yuan / base['total_cost_yuan']
    return row
...
def primary_stability(results):
    ...
    """
    g1 = next(r for r in results if r['totals']['strategy_id'] == PRIMARY_BASELINE)
    g2 = next(r for r in results if r['totals']['strategy_id'] == PRIMARY_TREATMENT)
    ...
    monthly.insert(0, 'baseline', PRIMARY_BASELINE)
    monthly.insert(0, 'treatment', PRIMARY_TREATMENT)
...
def protected_manifest():
    """Hash every pre-existing asset, excluding this round's own trees and shared append-only docs.

    The frozen module-14 helper excludes module 14's own prefixes, so it would treat this round's
    outputs as protected assets; this local copy uses this round's prefixes instead.
    """
    m = parent()
    manifest = {}
    for tree in m.PROTECTED_TREES:
        base = ROOT / tree
        if not base.exists():
            continue
...
def write_report(results, comparison, primary, contrasts, metrics, coverage, subsets, gate,
    ...
    lines = [        '# 问题二：LightGBM残差预测实验结果报告', '',
        ...
        ' `reports/问题二/实验方案/问题二_LightGBM残差预测实验方案.md` 登记的三组对照，并按接收核查意见补齐'
        ' 特征有效日前的朴素回退后新增两组修正组，合计五组全年因果回测；未锁定第二问最终模型，'
        ' 未填写 `result2.xlsx`，未修改第一问。', '',
        '## 1. 问题分析', '',
        '在相同的历史特征下，检验浅层梯度提升树能否利用非线性与特征交互改善残差预测，'
        '并降低既定调度系统的实际现金费用。五组只改变发布的功率预测：G0 为朴素负载与朴素光伏'
        '（统一门控）；G1 为已有联合岭回归加同一门控，G2 为联合 LightGBM 加同一门控，'
        '二者沿用 module 14 遗留的"特征有效日前未发布"缺口，其中 G1 复现 module 16 的 '
        'G3_joint_gate、G2 复现已签名结果；G3/G4 为 G1/G2 按方案回退朴素后的修正组，'
        f'构成主比较 {PRIMARY_TREATMENT} 减 {PRIMARY_BASELINE}。'
        '其余条件完全相同：公共 1 月预运行、W28 逐时段 q80、名义日末 6000 kWh、'
        '冻结 MILP 与因果贪心反馈。', '',
        '### 1.1 主要结论', '',
    ]
    for key, record in reference.items():
        if 'max_difference' in record:
            ...
            lines.append(f"- 旧组 `{record['label']}` 复现已签名结果：总费 "
                         f"{record['current_total_cost_yuan']:,.6f} 元，与签名值差 "
                         f"{record['difference_vs_signed_yuan']:.3e} 元，说明本次重构未改变旧行为。")
    for row in primary.itertuples():
        lines.append(f"- 主比较 `{row.treatment}` 减 `{row.baseline}`：总费 {row.delta_total_cost_yuan:+,.2f} 元"
                     f"（{row.delta_total_pct:+.4f}%），计划费差 {row.delta_planned_cost_yuan:+,.2f}，"
                     f"应急费差 {row.delta_emergency_cost_yuan:+,.2f}。")
    lines.append('- 本轮同时保留两组：旧组 G1/G2 沿用 module 14 遗留的"特征有效日前未发布"缺口，'
                 '修正组 G3/G4 按方案回退朴素；主比较与下列稳定性均以修正组 G3/G4 为准，'
                 'G0 两种口径下档案相同，作为匹配基准。')
    lines += ['', '## 2. 数据预处理', '',
    ...
    for sid in [g['id'] for g in GROUPS]:
        t = totals[sid]
        row = supp.loc[sid]
    ...
    s = primary_summary
    lines += ['', f'### 4.1 主比较稳定性（{PRIMARY_TREATMENT} 减 {PRIMARY_BASELINE}）', '',
    ...
    directions = []
    for target in ('load', 'pv', 'net_demand'):
        g1 = float(indexed.loc[(PRIMARY_BASELINE, target), 'mae'])
        g2 = float(indexed.loc[(PRIMARY_TREATMENT, target), 'mae'])
        directions.append(f"{target} {'改善' if g2 < g1 else '变差'} {abs(g2 - g1):,.4f} kW")
    lines.append(f"- 主比较 {PRIMARY_TREATMENT} 相对 {PRIMARY_BASELINE} 的点预测 MAE："
                 f"{'；'.join(directions)}。")
    lines.append('- 误差与费用并不同向：上表中未改善的目标不应宣称预测被改进，'
                 '费用变化只能归因于组合净需求与调度路径的联合结果。')
    lines += ['', '### 4.3 光伏后处理与子集', '',
              f"- **被门控归零的时段（即保留支持区间以外）** 共 {gate['gated_slots']} 段，"
              f"这些时段实际发电合计 {gate['gated_actual_energy_kWh']:,.4f} kWh，"
              f"最大实际功率 {gate['gated_actual_max_kW']:,.4f} kW，"
              f"误删（实际>100 kW 却被归零）{gate['gated_actual_gt100_slots']} 个时段；"
              '此能量是被删除的预测所覆盖的真实发电量，不是白天保留区间的发电量。']
    ...
    lines += ['', '### 4.4 训练与分支统计', '',
              f'说明：记录覆盖全年 365 个发布日。特征有效日之前回退朴素（不训练）；'
              f'"训练历史不足"指已进入特征有效期但窗口内有效日少于 {MIN_TRAIN_DAYS}；'
              f'"评价期内"指 2—12 月（day index ≥ {WARMUP_DAYS}），即 334 天的评价期。']
    log = pd.DataFrame(fit_log)
    for target in ('load', 'pv'):
        part = log[log.target == target]
        within = part[part.day_index >= WARMUP_DAYS]
        lines.append(
            f"- `{target}`：发布日合计 {len(part)}（特征有效日前回退 "
            f"{int((part.branch == 'feature_history_fallback').sum())}，"
            f"训练历史不足回退 {int((part.branch == 'insufficient_history').sum())}，"
            f"实际拟合 {int((part.branch == 'fitted').sum())}，"
            f"常数标签 {int((part.branch == 'constant_target').sum())}）；"
            f"其中评价期内实际拟合 {int((within.branch == 'fitted').sum())} 天，"
            f"实际树数合计 {int(part.n_trees.sum())}。")
    lines += ['', '### 4.5 验证范围与限制', '',
...
def main():
    ...
    if args.mode == 'register' or not registration_path.exists():
        record = dict(registered_utc=datetime.now(timezone.utc).isoformat(),
                      specification='reports/问题二/实验方案/问题二_LightGBM残差预测实验方案.md',
        ...
        if registration_path.exists():
            ...
            if existing['signature'] != signature:
                ...
                # Truly frozen spec: an amendment may not change these. The group roster and the
                # primary comparison are excluded because the receiving review explicitly
                # directed adding the gap-repaired pair alongside the legacy groups.
                for key in ('train_window_days', 'min_train_days', 'residual_window_days', 'theta',
                            'terminal_kWh', 'gate', 'lightgbm', 'evaluation_start'):
    ...
    for group in GROUPS:
        ...
        result = m.run_group(group, load, pv, price, dates, bundles[group['id']], warm_states,
                             verbose=(group['id'] == PRIMARY_TREATMENT))
        ...
        if group['id'] == LEGACY_TREATMENT:
            signed = abs(result['totals']['total_cost_yuan'] - REFERENCE_G2_LEGACY_YUAN)
            reference['G2_legacy'] = dict(label='G2_lightgbm_gate', signed_total_cost_yuan=
                                          REFERENCE_G2_LEGACY_YUAN,
                                          current_total_cost_yuan=result['totals']['total_cost_yuan'],
                                          difference_vs_signed_yuan=signed,
                                          passed=bool(signed <= REFERENCE_COST_TOL_YUAN))
            assert reference['G2_legacy']['passed'], reference['G2_legacy']
            print(f'    legacy G2 reproduces the signed run (diff {signed:.3e} CNY)', flush=True)
    ...
    primary = contrast(comparison, PRIMARY_TREATMENT, PRIMARY_BASELINE)
    frame_to_csv(primary, OUT / 'primary_contrasts.csv')
    legacy = contrast(comparison, LEGACY_TREATMENT, LEGACY_BASELINE)
    frame_to_csv(legacy, OUT / 'legacy_contrasts.csv')
    ...
    primary_result = next(r for r in results if r['totals']['strategy_id'] == PRIMARY_TREATMENT)
    perturbation = perturbation_checks(load, pv, price, dates, lightgbm_forecasts, bundles,
                                       warm_states, primary_result)
    save(OUT / 'future_perturbation_checks.json', perturbation)
    prefix = feedback_prefix_check(load, pv, price, dates, lightgbm_forecasts, bundles,
                                   primary_result)
