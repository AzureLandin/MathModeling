...
Specification: reports/问题二_岭回归残差预测实验方案.md (single-run task book, not a new main model).
...
PLAN_MD = ROOT / 'reports' / '问题二_岭回归残差预测实验方案.md'
REPORT_MD = ROOT / 'reports' / '问题二_岭回归残差预测实验结果报告.md'
...
def register(hashes, snapshot_hashes, protected, amend_reason):
    record = dict(
        ...
        specification='reports/问题二_岭回归残差预测实验方案.md',
        specification_sha256=hashes['reports/问题二_岭回归残差预测实验方案.md'],
...
def write_report(results, comparison_table, contrasts, energy, stability, metrics, coverage,
    ...
    lines = [
        ...
        ' `reports/问题二_岭回归残差预测实验方案.md` 登记的四组全年因果回测；未锁定第二问最终模型，'
