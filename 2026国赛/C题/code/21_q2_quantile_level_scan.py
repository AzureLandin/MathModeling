...
Specification: reports/问题二_分位水平扫描实验方案.md (single-run sensitivity task book).
...
PLAN_MD = ROOT / 'reports' / '问题二_分位水平扫描实验方案.md'
REPORT_MD = ROOT / 'reports' / '问题二_分位水平扫描实验结果报告.md'
...
OWN_NEW_REL = {'code/21_q2_quantile_level_scan.py', 'reports/问题二_分位水平扫描实验结果报告.md'}
...
def write_report(summary, within, between, monthly, minima, stability, coverage, metrics,
    ...
    lines = [
        ...
        ' `reports/问题二_分位水平扫描实验方案.md` 登记的十组情景；未训练任何预测器，未锁定最终策略，'
...
def main():
    ...
    if args.mode == 'register' or not registration_path.exists():
        record = dict(registered_utc=datetime.now(timezone.utc).isoformat(),
                      specification='reports/问题二_分位水平扫描实验方案.md',
