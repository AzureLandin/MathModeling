...
依据式~\eqref{eq:q1_full_model}构建MILP模型，定义目标函数和全部约束条件，调用HiGHS求解器进行求解。求解器在$0.034$秒内完成求解，搜索节点数为1，返回全局最优解，相对间隙为$0.000\times 10^{0}$，证实了解的全局最优性。目标函数下界与最优值完全一致，均为$35\,126.95$元。
