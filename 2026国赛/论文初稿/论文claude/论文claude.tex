...
\begin{center}
{\zihao{-2}\heiti\bfseries 光伏预测与波动电价下的微网滚动MILP购电策略优化}
\end{center}
\vspace{1em}
...
\textbf{针对问题二：}在负载与光伏实时变化且供电不足时按5倍电价应急购电的条件下，本文先依据周内规律与日间规律构造季节性朴素预测，再用LightGBM回归模型修正负载与光伏预测，并以历史残差$q_{80}$分位数补偿预测低估风险，建立以计划购电费用最小为目标的日前混合整数线性规划模型；执行阶段按实际供需逐时段反馈储能并核算应急购电。自由名义末态方案在$2025$年$2$--$12$月评价期内总费用为$13\,699\,337.68$元，较固定$6\,000$~kWh名义末态方案节省$0.2091\%$，应急购电费占比仅为$3.17\%$。

\textbf{针对问题三：}在问题二日前计划基础上引入$0{:}00$、$6{:}00$、$12{:}00$和$18{:}00$发布的附件3光伏预报，并在日内更新节点对尚未交付时段进行带调整费用的滚动修订。最终方案采用F2日内负载修正、Linear光伏转换与自身$q_{75}$历史残差保护，形成$0{:}00$计划、日内择优进而逐段反馈的滚动混合整数线性规划框架。相比仅使用原$0{:}00$负载预测的S0方案，最终S2方案总费用由$13\,353\,822.04$元降至$13\,330\,377.96$元，降幅为$0.1756\%$，其中应急购电费由$186\,246.70$元降至$105\,049.17$元，应急电量下降$44.1\%$。

\textbf{针对问题四：}在附件4电价逐日逐段波动并将每日计划范围内电价视为已知输入的假设下，对问题二和问题三重新求解。$4$--$2$采用Q42方案，按日前混合整数线性规划在波动电价下重新优化，总费用为$14\,347\,907.89$元；$4$--$3$采用Q43\_S2方案，在问题三滚动修订框架下按交付区间电价重新优化，总费用为$13\,972\,808.91$元，较Q42节省$375\,098.98$元，降幅为$2.6143\%$。与固定计划回放相比，三条主分支重新优化均更省，但普通购电节省的同时调整费与应急费上升，说明节费与应急风险应并重评价。
\end{abstract}
...
\subsection{问题四的分析}
问题四要求将第二问和第三问的购电策略放到附件4逐日波动的电价下重新求解。与前三问不同，电价不再是每天重复的固定序列，而是随日期和$10$分钟时段变化并存在明显尖峰与低谷，因此储能跨时段充放电的经济价值会发生改变。本文按已确认口径，将每日$0{:}00$计划范围内的$144$个交付区间电价视为已知输入，不建立电价预测模型，也不增加题面未给出的价格修正机制。

本问在沿用前问信息权限的前提下只更换价格模块：$4$--$2$延续问题二每天$0{:}00$制定日前计划的决策结构，$4$--$3$延续问题三在$6{:}00$、$12{:}00$、$18{:}00$滚动修订尚未交付时段的决策结构，所有求解、择优与结算均按交付区间电价进行。为分离重新优化与固定计划的差异，本文设置六组对照，以现金总费用、应急购电和期末库存为主要指标，评价波动电价下重新优化与滚动修订的实际价值。
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{../../C题/问题一可能用到的图片/-问题一.drawio.png}
  \caption{问题一模型求解流程}
...
\noindent 储能设备的容量、功率与效率等特性决定了调度可行性\upcite{chen2012microgrid}。\textbf{b. 储能设备储电量递推约束}\\[4pt]
...
本问采用优化求解器HiGHS对上述MILP模型进行精确求解。由于目标函数与全部约束均为线性，充放电互斥只需引入0-1变量即可线性刻画，问题一属于标准的混合整数线性规划，采用分支定界法\upcite{wolsey1998integer}即可获得全局最优解；HiGHS集成了单纯形法、内点法和分支定界法，能够高效处理该规模共865个变量的MILP，对该尺寸问题可在秒级内精确求解并获得全局最优证认，无需依赖遗传算法等随机搜索方法。
...
全天购电策略的核心特征可以从时序图中直接观察。图~\ref{fig:q1_dispatch_overview}给出全天购电功率、充电功率与放电功率的变化，图~\ref{fig:q1_soc_trajectory}给出储电量全天变化与运行边界。购电功率在凌晨谷价时段维持高位，白天光伏盈余时段降为零，傍晚高价时段由储能放电替代购电，深夜再恢复购电补充储能，形成"谷充峰放"的调度时序。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{../../C题/问题一可能用到的图片/全天调度总览.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{../../C题/figures/q1_paper/q1_fig1_soc_trajectory.png}
  \caption{问题一最优调度下的储电量变化与运行边界}
  \label{fig:q1_soc_trajectory}
\end{figure}

图~\ref{fig:q1_dispatch_overview}中放电功率显示为负值，表示储能向母线供电；图~\ref{fig:q1_soc_trajectory}中储电量始终位于$1200$--$10800$~kWh范围内，日末回到$6000$~kWh，与表~\ref{tab:q1_storage}结果一致。

\subsubsection{结果分析与验证}

\paragraph{（1）最优策略的经济学解释}

微网系统经济运行优化是储能参与下的经典问题\upcite{ding2011microgrid}。图~\ref{fig:q1_dispatch_overview}与图~\ref{fig:q1_soc_trajectory}所示的调度时序本质上是\textbf{通过储能设备实现电力的时间套利}：凌晨谷价时段大量购电并充电，白天光伏盈余时段停止购电，傍晚高价时段依靠储能放电覆盖负载，深夜再恢复购电补充储能。以$\eta_c \cdot \eta_d = 0.81$的往返效率为代价，低价电力被转移至高价时段使用；储能全天充电量为$20\,740.67$~kWh、放电量为$16\,799.94$~kWh，其差值$3\,940.73$~kWh即为往返能量损耗。
...
问题二整体求解流程如图~\ref{fig:q2_flow}所示。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{../../C题/问题二可能用到的图片/流程图.pdf}
  \caption{问题二日前滚动求解流程}
  \label{fig:q2_flow}
\end{figure}
...
由于紧急购电按$5$倍电价计费，代价远高于计划购电，需对净需求预测进行上行修正。本文基于大数据报童模型的最优分位数理论\upcite{ban2019newsvendor}推导保护水平。
...
日前MILP模型\upcite{silva2020dayahead}为：
\begin{equation}
\begin{aligned}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{../../C题/问题二可能用到的图片/q2_fig1_annual_cost_and_emergency_share.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.48\textwidth]{../../C题/问题二可能用到的图片/q2_fig4_monthly_cost_differences.png}
...
问题三整体滚动求解流程如图~\ref{fig:q3_flow}所示：每天先由$0{:}00$计划冻结全天原始购电方案，之后三个更新节点只对尚未交付的区间重优化，实际执行阶段逐段反馈并跨日衔接。

\noindent 本文采用如下方案编号。字母$B$表示问题三首轮对照基准（Baseline），$B_0$、$B_1$、$B_2$分别用于比较问题二基准、更换附件3的$0{:}00$光伏预报以及引入日内滚动修订的效果。字母$S$表示最终费用比较中的策略方案（Strategy），$S_0$采用原始$0{:}00$负载预测，$S_2$采用F2日内负载修正；$S_1$虽对应F1历史偏差修正，但未进行完整费用回测。字母$Q$用于问题四波动电价下的重新优化方案，下标的第一位$4$表示问题四，第二位$2$或$3$表示沿用问题二或问题三的模型框架。这些字母均为方案标签，不是模型决策变量。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\textwidth]{../../C题/问题三可能用到的图片/流程图3.drawio.png}
  \caption{问题三滚动求解流程}
  \label{fig:q3_flow}
\end{figure}
...
问题三在问题二预测框架基础上引入附件3光伏预报版本管理与日内负载修正。历史净需求残差按发布小时与目标时段分组，避免不同发布版本的误差分布相互混合。对当前发布时刻$r$与目标区间$t$，定义预测净需求、预测残差与保护性净需求：
\begin{equation}
...
\end{equation}
\noindent 其中$\widehat{L}_{r,t}$沿用问题二的$15$特征LightGBM负载预测，$n_t=(L_t-V_t)\Delta t$为实际净需求。S0方案保持原$0{:}00$负载预测，S2方案在$6{:}00$、$12{:}00$、$18{:}00$用F2方法对负载预测作当日偏差修正，光伏预测统一采用附件3的Linear转换。对每组样本取此前$28$个发布日内已实现的残差，有效样本数$m\geq7$时取升序第$\lceil0.75m\rceil$个残差作为$\rho_{r,t}$，样本不足$7$个时回退零修正。两组各自基于自身发布误差建立$q_{75}$保护，该分位水平由费用对照实验确认，不宣称其在新增调整价格下的全局最优性。

\subsubsection{模型建立}

\noindent 考虑到日内多阶段调度是提升微网应对供需变化能力的常用手段\upcite{yan2018intraday}，本文在$0{:}00$生成原始计划后保留三个日内更新节点。记$\mathcal{D}_k$为第$k$日$0{:}00$计划覆盖的$144$个时段，$\mathcal{F}_r$为更新时刻$r$可调整的未来区间集合，$p_t$为电量交付区间的正常电价，$q_t^{0}$、$q_t^{r}$、$q_t^{\mathrm{eff}}$分别为$0{:}00$原始计划、$r$时刻候选计划与交付前最后一个合法版本的有效购电量。$0{:}00$时，在问题二相同的储能物理约束下，以最小化计划购电费用为目标求解原始计划：
\begin{equation}
...
\noindent 三组最终储电量相同，真实用电量为$1\,682.90$~kWh、计划购电量为$1\,642.19$~kWh，初始库存也相同，差异完全来自运行过程中的购电与应急行为。$B_1$较$B_0$总费用增加$148\,749.31$元，增幅为$1.0858\%$，其中普通购电费增加$187\,588.17$元、应急费减少$38\,838.85$元；从月度看$B_1$仅在$3/11$个月更省，逐月正负交替，说明单独把$0{:}00$预报更换为附件3版本在本配置下不是稳健改进。该差额同时包含预报来源与提前时长口径的变化，不能据此断言附件3预报"更差"。
...
\noindent 从储电量变化看，$6$月$21$日光伏充足，日末储电升至$3\,045.09$~kWh；$12$月$21$日冬季高负载，日末储电降至$1\,798.21$~kWh；$3$月$20$日与$9$月$23$日日末储电分别为$1\,729.90$和$1\,636.19$~kWh。储能始终被用于低谷与光伏富余时段充电、高负载时段放电，与问题一的"谷充峰放"规则一致。
...
为验证滚动求解链路的可靠性，本问完成三类精简验证：在结算算例中，设$q^{0}=100$~kWh、$p=1$元/kWh，最终有效电量依次取$0$、$80$、$100$、$120$~kWh时，总费用分别为$50$、$90$、$100$、$130$元，减购与增购边界均与式~\eqref{eq:q3_cash}一致；在$2025$年$6$月$21$日信息边界检查中，对$0{:}00$决策时$28\,131$个未实现真值格、$6{:}00$决策时$28\,094$个未实现真值格按$\times1.05$缩放，并给尚未发布的预报增加$50$~kW后，重建整条预测与保护链路得到的计划与接受决策逐位一致；跨求解器一致性检查中，该日$0{:}00$计划目标值差为$3.6\times10^{-12}$元、计划最大差为$2.3\times10^{-13}$~kWh。逐段结算恒等式残差不超过$9.1\times10^{-13}$元，实际储能物理守恒残差不超过$4.6\times10^{-12}$~kWh。
...
% ======================== 问题三最终方案费用对照 ========================
\subsubsection{最终方案的费用对照}

在上述滚动修订框架基础上，本文进一步比较S0与S2两套候选方案。S0采用原$0{:}00$负载预测、Linear光伏与自身$q_{75}$保护；S2在三个日内更新节点用F2方法修正负载预测，并以F2自身发布误差重建$q_{75}$保护。两组均沿用四节点滚动更新、真实储能反馈与最终一次结算口径，自然账本费用分解如表~\ref{tab:q3_s2_comparison}所示。

\begin{table}[H]
\centering
\caption{问题三S0与S2方案的全年费用与应急指标}
\label{tab:q3_s2_comparison}
\zihao{5}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lccccc}
\toprule
\textbf{方案} & \textbf{普通购电费（元）} & \textbf{调整费（元）} & \textbf{应急费（元）} & \textbf{总费用（元）} & \textbf{应急电量（kWh）} \\
\midrule
S0 & $12\,771\,312.06$ & $396\,263.27$ & $186\,246.70$ & $13\,353\,822.04$ & $36\,526.9$ \\
S2 & $12\,772\,385.33$ & $452\,943.46$ & $105\,049.17$ & $\bm{13\,330\,377.96}$ & $20\,406.7$ \\
\bottomrule
\end{tabular}}
\end{table}

\noindent S2较S0节约$23\,444.08$元，降幅为$0.1756\%$，节费主要来自应急购电费减少$81\,197.53$元，同时普通购电费增加$1\,073.26$元、调整费增加$56\,680.18$元。S2的应急电量由$36\,526.9$~kWh降至$20\,406.7$~kWh，降幅为$44.1\%$；S2的$1\,002$次日内更新全部被采用，S0为$902$次。图~\ref{fig:q3_s2_cost}给出两方案的现金三项差与应急指标，图~\ref{fig:q3_s2_monthly}给出逐月现金费用差。S2仅在$5/11$个月更省，净节费集中于少数日期，因此本文将其表述为当前评价期和固定配置下费用较低的最终候选，不宣称跨年度全局最优。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.66\textwidth]{../../C题/figures/q3_intraday_load_cost/cost_components_and_emergency.png}
  \caption{问题三S2相对S0的现金分项与应急指标对比}
  \label{fig:q3_s2_cost}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.66\textwidth]{../../C题/figures/q3_intraday_load_cost/monthly_cash_difference.png}
  \caption{S2相对S0逐月现金费用差}
  \label{fig:q3_s2_monthly}
\end{figure}

图~\ref{fig:q3_s2_soc}给出最终S2方案四个指定日期的储电量变化，图中带颜色的区间表示充电或放电时段，储电量始终保持在物理边界内。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\textwidth]{../../C题/figures/q3_paper/q3_fig1_selected_dates_soc.png}
  \caption{S2方案四个指定日期的储电量变化曲线}
  \label{fig:q3_s2_soc}
\end{figure}

\subsection{问题四模型建立及求解}

问题四整体求解流程如图~\ref{fig:q4_flow}所示。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\textwidth]{../../C题/问题四可能用到的图片/问题四求解流程.drawio.png}
  \caption{问题四滚动求解流程}
  \label{fig:q4_flow}
\end{figure}
...
在分布式能源系统集成持续发展的背景下\upcite{li2023review}，波动电价下的购电与储能调度成为配电网运行的重要决策问题；柔性负荷接入还会影响负荷侧的可调度空间\upcite{qi2020distribution}，需要在评价时统筹考虑。附件4给出$2025$年每天从$00{:}10$至次日$00{:}00$以及$0{:}00{+}1$共$144$个交付区间的电价。本文沿用前三问的时间口径，$00{:}10$仍表示$00{:}10$--$00{:}20$区间，模板第$k$行按真实日期与区间起点映射到附件4交付电价，次日$00{:}00$--$00{:}10$取前一日期行末尾电价。同一交付区间在$0{:}00$、$6{:}00$、$12{:}00$、$18{:}00$规划时使用同一个价格，优化目标、新旧方案择优与最终结算三处均按该价格计费。由于题面未给出电价公布机制，本文按已确认口径将每日$0{:}00$计划范围内的$144$段电价视为已知输入，不建立电价预测模型或随机电价情景。

以附件4电价重新生成公共$1$月物理初始化，$4$--$2$与$4$--$3$共用$2025$年$2$月$1$日$0{:}00$真实库存$6\,097.354410$~kWh及午夜承诺，$1$月仅用于历史积累，不计入正式评价。预测与保护档案直接复用前问冻结结果：$4$--$2$沿用问题二$15$特征负载、光伏预测与$W=28$日$q_{80}$保护；$4$--$3$分别保留S0与S2两套候选，S0采用原$0{:}00$负载预测、Linear光伏和自身$q_{75}$保护，S2采用F2日内负载修正、Linear光伏和F2自身$q_{75}$保护。更换电价不重新训练预测模型，也不重建保护分位。

\subsubsection{模型建立}

令$p_t$为交付区间$t$的实际电价，$q_t^{0}$、$q_t^{r}$、$q_t^{\mathrm{eff}}$分别表示$0{:}00$原始计划、$r$时刻候选计划与最终有效购电量，$e_t$为实际应急购电量。保护性净需求沿用第三问定义：
\begin{equation}
  \widetilde{n}_{t|r}=(\widehat{L}_{t|r}-\widehat{V}_{t|r})\Delta t+\rho_{t|r}
  \label{eq:q4_protected}
\end{equation}
\noindent 其中$\widehat{L}_{t|r}$、$\widehat{V}_{t|r}$分别为当前发布时刻可用的负载与光伏预测，$\rho_{t|r}$为按"发布小时--目标时段"分组的历史净需求残差分位数。名义储能的母线平衡、状态递推、容量边界与充放电互斥约束沿用式~\eqref{eq:q3_balance}和式~\eqref{eq:q3_physical}，名义初态取当前真实储电量，名义末态采用自由末态。

$4$--$2$在每天$0{:}00$以最小化计划购电费为目标求解日前计划：
\begin{equation}
  \min \quad \sum_{t\in\mathcal{T}_k} p_t q_t^{0}
  \label{eq:q4_obj42}
\end{equation}
\noindent $4$--$3$在$0{:}00$求解相同的原始计划，并在$r\in\{6{:}00,12{:}00,18{:}00\}$对剩余可调整区间$\mathcal{F}_r$重新决策，目标为普通购电费与相对原始计划的调整费之和：
\begin{equation}
  \min \quad \sum_{t\in\mathcal{F}_r}
  p_t\left[q_t^{r}+\frac{1}{2}\left|q_t^{r}-q_t^{0}\right|\right]
  \label{eq:q4_obj43r}
\end{equation}
\noindent 每次更新后按式~\eqref{eq:q3_score}在相同最新预测、相同真实库存、相同剩余区间与相同交付电价下评价新旧计划，新候选严格更便宜才覆盖旧版本，平局保留旧版本。

实际执行沿用逐段贪心储能反馈：当有效购电量高于实际净需求时先充电，剩余电量记为未使用量；当有效购电量低于实际净需求时先放电，剩余缺口按$5$倍电价应急购电。实际现金总费用按自然日与模板两套账本分别结算：
\begin{equation}
  C_{42}=\sum_t p_t\left(q_t^{0}+5e_t\right)
  \label{eq:q4_cash42}
\end{equation}
\begin{equation}
  C_{43}=\sum_t p_t\left(q_t^{\mathrm{eff}}+\frac{1}{2}\left|q_t^{\mathrm{eff}}-q_t^{0}\right|+5e_t\right)
  \label{eq:q4_cash43}
\end{equation}
\noindent $4$--$3$中间被覆盖的版本不重复收费，所有费用只相对所属模板日$0{:}00$原始计划最终结算一次。

\subsubsection{模型求解}

本文在附件4电价和共用公共初始化下运行六组对照：F42与Q42对应$4$--$2$，F43\_S0、Q43\_S0、F43\_S2、Q43\_S2对应$4$--$3$的S0与S2两套候选。F组只冻结前问普通购电计划并从新初态重新执行储能反馈，不再重新求解；Q组在附件4电价下重新优化，其中Q43\_S0与Q43\_S2各经历$334$次$0{:}00$计划求解和$1\,002$次日内更新决策。求解仍通过SciPy接口调用HiGHS求解器，相对MIP gap目标为$10^{-9}$，单次时间上限$120$秒。

\subsubsection{结果展示}

六组方案在$2025$年$2$--$12$月自然账本中的现金费用如表~\ref{tab:q4_cost}所示。

\begin{table}[H]
\centering
\caption{问题四六组方案在附件4电价下的全年费用分解}
\label{tab:q4_cost}
\zihao{5}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lccccc}
...
\textbf{方案} & \textbf{普通购电费（元）} & \textbf{调整费（元）} & \textbf{应急费（元）} & \textbf{总费用（元）} & \textbf{应急电量（kWh）} \\
\midrule
F42     & $13\,998\,469.03$ & $0.00$          & $465\,034.55$ & $14\,463\,503.57$ & $73\,096.0$ \\
Q42     & $13\,872\,116.16$ & $0.00$          & $475\,791.73$ & $14\,347\,907.89$ & $74\,524.1$ \\
F43\_S0 & $13\,485\,052.22$ & $420\,809.23$   & $208\,104.55$ & $14\,113\,966.00$ & $36\,526.9$ \\
Q43\_S0 & $13\,362\,493.88$ & $421\,385.54$   & $209\,793.87$ & $13\,993\,673.29$ & $38\,071.8$ \\
F43\_S2 & $13\,486\,108.15$ & $479\,056.02$   & $117\,808.76$ & $14\,082\,972.92$ & $20\,406.7$ \\
Q43\_S2 & $13\,362\,355.91$ & $480\,740.86$   & $129\,712.14$ & $\bm{13\,972\,808.91}$ & $23\,567.6$ \\
\bottomrule
\end{tabular}}
\end{table}

\noindent 三条主分支相对各自固定计划回放均更省：$S_{42}=+115\,595.68$元，$S_{43\_S0}=+120\,292.72$元，$S_{43\_S2}=+110\,164.02$元。节费主要来自重新优化带来的普通购电减少，但调整费和应急费同时上升，不能把总费用下降读成应急风险改善。Q43\_S2为四问最低总费用方案，较Q42节省$375\,098.98$元，降幅为$2.6143\%$；该差额同时包含光伏预报来源、日内更新权限与保护水平差异，不是单纯的光伏信息价值。图~\ref{fig:q4_cost_savings}给出六组费用分解与三个节费量的对比。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.66\textwidth]{../../C题/figures/q4_price_transfer/cost_components_and_savings.png}
  \caption{问题四六组费用分解与重新优化节费量}
  \label{fig:q4_cost_savings}
\end{figure}

图~\ref{fig:q4_monthly}给出Q43\_S2相对Q42的逐月节费与费用分项分解，可以看出两种方案的差额在全年各月持续存在，节费不是由少数异常日单独造成。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.66\textwidth]{../../C题/figures/q4_paper/q4_fig1_monthly_savings_and_decomposition.png}
  \caption{Q43\_S2相对Q42的逐月节费与费用分项分解}
  \label{fig:q4_monthly}
\end{figure}

三条主分支的应急指标如表~\ref{tab:q4_emergency}所示。Q43\_S2的应急电量最低，但应急天数和连续事件数高于Q42，说明滚动修订减少的是应急深度而非应急发生频率。

\begin{table}[H]
\centering
\caption{问题四三条主分支的应急指标}
\label{tab:q4_emergency}
\zihao{5}
\begin{tabular}{lccccc}
\toprule
\textbf{方案} & \textbf{应急电量（kWh）} & \textbf{应急区间数} & \textbf{应急天数} & \textbf{连续事件数} & \textbf{应急费（元）} \\
\midrule
Q42     & $74\,524.1$ & $622$ & $132$ & $243$ & $475\,791.73$ \\
Q43\_S0 & $38\,071.8$ & $726$ & $142$ & $290$ & $209\,793.87$ \\
Q43\_S2 & $23\,567.6$ & $618$ & $157$ & $299$ & $129\,712.14$ \\
\bottomrule
\end{tabular}
\end{table}

图~\ref{fig:q4_selected_day}以$2025$年$6$月$21$日为例给出电价、普通购电与实际储电量的对应关系。Q43分支在$6{:}00$、$12{:}00$、$18{:}00$修订后会在低价时段提高购电、高价时段更多依赖储能放电，购电曲线出现明显的日内台阶；Q42只有$0{:}00$一次计划，曲线保持稳定。价格与净需求同时变化，本文不将这一行为解释为单一电价因素的结果。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{../../C题/figures/q4_price_transfer/selected_day_dispatch.png}
  \caption{$2025$年$6$月$21$日电价、普通购电与实际储电量对应关系}
  \label{fig:q4_selected_day}
\end{figure}

按题目要求，表~\ref{tab:q4_specified_purchase}给出Q42与Q43\_S2在四个指定日期指定$10$分钟时段的最终有效购电量及全天购电量与总费用，表~\ref{tab:q4_specified_storage}给出Q43\_S2的储能汇总结果。

\begin{table}[H]
\centering
\caption{问题四指定日期指定时段的最终有效购电量及全天购电量与总费用}
\label{tab:q4_specified_purchase}
\zihao{5}
\resizebox{\textwidth}{!}{%
\begin{tabular}{c|c c c c c c|cc}
\toprule
\textbf{日期 / 方案} & $10{:}00$ & $12{:}00$ & $14{:}00$ & $16{:}00$ & $18{:}00$ & $20{:}00$ & \textbf{全天购电量（kWh）} & \textbf{全天总费用（元）} \\
\midrule
3.20  Q42     & $0.00$ & $640.55$ & $0.00$ & $447.48$ & $736.81$ & $725.52$ & $68\,624.22$ & $42\,771.26$ \\
3.20  Q43\_S2 & $0.00$ & $495.78$ & $0.00$ & $496.67$ & $730.67$ & $717.53$ & $65\,756.12$ & $43\,014.57$ \\
6.21  Q42     & $0.00$ & $0.00$    & $0.00$ & $144.98$ & $434.65$ & $0.00$    & $36\,464.73$ & $18\,007.63$ \\
6.21  Q43\_S2 & $0.00$ & $0.00$    & $0.00$ & $109.49$ & $430.66$ & $0.00$    & $36\,500.26$ & $17\,851.63$ \\
9.23  Q42     & $0.00$ & $539.99$ & $0.00$ & $442.32$ & $793.44$ & $304.22$ & $68\,821.64$ & $47\,038.07$ \\
9.23  Q43\_S2 & $0.00$ & $390.36$ & $0.00$ & $458.18$ & $787.24$ & $288.60$ & $66\,615.13$ & $45\,378.25$ \\
12.21 Q42     & $0.00$ & $1\,047.81$ & $0.00$ & $809.84$ & $727.88$ & $0.00$ & $97\,024.51$ & $73\,749.00$ \\
12.21 Q43\_S2 & $0.00$ & $881.65$  & $0.00$ & $827.40$ & $718.36$ & $0.00$ & $94\,615.28$ & $72\,970.07$ \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{table}[H]
\centering
\caption{问题四Q43\_S2指定日期储能设备六个$4$小时时段的充放电量及首尾储电量}
\label{tab:q4_specified_storage}
\zihao{5}
\resizebox{\textwidth}{!}{%
\begin{tabular}{c|cc|cc|cc|cc|cc|cc|cc}
\toprule
\textbf{日期} & \multicolumn{2}{c|}{$0{:}00$--$4{:}00$} & \multicolumn{2}{c|}{$4{:}00$--$8{:}00$} & \multicolumn{2}{c|}{$8{:}00$--$12{:}00$} & \multicolumn{2}{c|}{$12{:}00$--$16{:}00$} & \multicolumn{2}{c|}{$16{:}00$--$20{:}00$} & \multicolumn{2}{c|}{$20{:}00$--$24{:}00$} & \multicolumn{2}{c}{$0{:}00$ / $24{:}00$储电量} \\
 & 充 & 放 & 充 & 放 & 充 & 放 & 充 & 放 & 充 & 放 & 充 & 放 & (kWh) & (kWh) \\
\midrule
3.20  & $6\,348$ & $728$  & $3\,438$ & $5\,488$ & $3\,471$ & $3\,195$ & $7\,178$ & $255$   & $437$  & $6\,477$ & $1\,229$ & $2\,928$ & $2\,849.28$ & $1\,550.65$ \\
6.21  & $2\,086$ & $1\,487$ & $2\,810$ & $1\,720$ & $9\,648$ & $0$     & $0$     & $0$     & $39$   & $5\,142$ & $1\,628$ & $3\,065$ & $1\,273.47$ & $3\,181.30$ \\
9.23  & $6\,182$ & $55$   & $3\,435$ & $5\,737$ & $4\,444$ & $2\,388$ & $6\,045$ & $415$   & $153$  & $6\,588$ & $379$  & $2\,261$ & $2\,254.76$ & $1\,446.74$ \\
12.21 & $7\,740$ & $771$  & $2\,548$ & $74$    & $4\,402$ & $8\,337$ & $8\,611$ & $2\,224$ & $39$   & $4\,444$ & $315$  & $4\,180$ & $2\,479.20$ & $1\,512.40$ \\
\bottomrule
\end{tabular}}
\end{table}
...
问题四的结论建立在已知交付电价、共用公共初始化和确定性反馈模型三个前提下。Q42与Q43\_S2均只代表当前$2025$年评价期和固定配置下的费用比较，不表述为跨年度全局最优；Q43\_S2相对Q43\_S0的差额是负载修正、保护重建与调度响应共同作用的组合效果，不拆分为单一收益。与固定计划回放相比，重新优化均降低总费用，但应急电量、应急天数和连续事件数上升，说明以总费用为目标时应当同时报告应急风险。三类精简验证确认了价格映射、求解与反馈的数值一致性，逐段能量和费用残差均小于$10^{-6}$~kWh和$10^{-4}$元量级。
...
本参赛队在竞赛过程中使用了AI工具，主要用于赛题理解、模型推导辅助、代码调试、文献检索与语言润色，详细使用情况见支撑材料。
...
\bibitem{ke2017lightgbm} Ke G L, Meng Q, Finley T, et al. LightGBM: A highly efficient gradient boosting decision tree[A]//Advances in Neural Information Processing Systems 30[C]. Red Hook: Curran Associates, 2017: 3149-3157.

\bibitem{chen2012microgrid} 陈健，王成山，赵波，张雪松. 考虑储能系统特性的独立微电网系统经济运行优化[J]. 电力系统自动化，2012,36(20):25-31.

\bibitem{wolsey1998integer} Wolsey L A. Integer programming[M]. New York: John Wiley \& Sons, 1998.

\bibitem{ding2011microgrid} 丁明，张颖媛，茆美琴，刘小平，徐宁舟. 包含钠硫电池储能的微网系统经济运行优化[J]. 中国电机工程学报，2011,31(04):7-14.

\bibitem{ban2019newsvendor} Ban G Y, Rudin C. The big data newsvendor: Practical insights from machine learning[J]. Operations Research, 2019, 67(1): 90-108. DOI:10.1287/opre.2018.1757.

\bibitem{silva2020dayahead} Silva V A, Aoki A R, Lambert-Torres G. Optimal day-ahead scheduling of microgrids with battery energy storage system[J]. Energies, 2020, 13(19): 5188. DOI:10.3390/en13195188.

\bibitem{yan2018intraday} 颜宁，潘霄，张明理，马少华，厉伟，张博. 基于复合储能的多互联微网日内调度研究[J]. 电工技术学报，2018,33(S2):577-585.

\bibitem{li2023review} 李承周，王宁玲，窦潇潇，杨志平，王利刚，杨勇平. 多能源互补分布式能源系统集成研究综述及展望[J]. 中国电机工程学报，2023,43(18):7127-7150.

\bibitem{qi2020distribution} 齐宁，程林，田立亭，郭剑波，黄仁乐，王存平. 考虑柔性负荷接入的配电网规划研究综述与展望[J]. 电力系统自动化，2020,44(10):193-207.
