...
本问采用优化求解器HiGHS\upcite{huangfu2018parallelizing}对上述MILP模型进行精确求解。由于目标函数与全部约束均为线性，充放电互斥只需引入0-1变量即可线性刻画，问题一属于标准的混合整数线性规划\upcite{wolsey1998integer}，采用分支定界法即可获得全局最优解；HiGHS集成了单纯形法、内点法和分支定界法，能够高效处理该规模共865个变量的MILP，对该尺寸问题可在秒级内精确求解并获得全局最优证认，无需依赖遗传算法等随机搜索方法。
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{../../C题/figures/q1_paper/q1_fig1_soc_trajectory.png}
...
图~\ref{fig:q1_dispatch_overview}与图~\ref{fig:q1_soc_trajectory}所示的调度时序本质上是\textbf{通过储能设备实现电力的时间套利}\upcite{ding2011microgrid,yan2018intraday,chen2012microgrid,li2023review,qi2020distribution}：凌晨谷价时段大量购电并充电，白天光伏盈余时段停止购电，傍晚高价时段依靠储能放电覆盖负载，深夜再恢复购电补充储能。以$\eta_c \cdot \eta_d = 0.81$的往返效率为代价，低价电力被转移至高价时段使用；储能全天充电量为$20\,740.67$~kWh、放电量为$16\,799.94$~kWh，其差值$3\,940.73$~kWh即为往返能量损耗。
...
\begin{equation}
  \min \quad \sum_{t\in\mathcal{D}_k} p_t q_t^{0}
  \label{eq:q3_obj0}
\end{equation}
...
整体滚动求解流程如图~\ref{fig:q3_flow}所示：每天先由$0{:}00$计划冻结全天原始购电方案，之后三个更新节点只对尚未交付的区间重优化，实际执行阶段逐段反馈并跨日衔接。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\textwidth]{../../C题/figures/问题三求解流程/问题三求解流程.png}
  \caption{问题三滚动求解流程}
  \label{fig:q3_flow}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\textwidth]{../../C题/figures/q3_intraday_load_cost/cost_components_and_emergency.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\textwidth]{../../C题/figures/q3_intraday_load_cost/monthly_cash_difference.png}
...
附件4给出$2025$年每天从$00{:}10$至次日$00{:}00$以及$0{:}00{+}1$共$144$个交付区间的电价。本文沿用前三问的时间口径，$00{:}10$仍表示$00{:}10$--$00{:}20$区间，模板第$k$行按真实日期与区间起点映射到附件4交付电价，次日$00{:}00$--$00{:}10$取前一日期行末尾电价。同一交付区间在$0{:}00$、$6{:}00$、$12{:}00$、$18{:}00$规划时使用同一个价格，优化目标、新旧方案择优与最终结算三处均按该价格计费。由于题面未给出电价公布机制，本文按已确认口径将每日$0{:}00$计划范围内的$144$段电价视为已知输入，不建立电价预测模型或随机电价情景。
...
本文在附件4电价和共用公共初始化下运行六组对照：F42与Q42对应$4$--$2$，F43\_S0、Q43\_S0、F43\_S2、Q43\_S2对应$4$--$3$的S0与S2两套候选。F组只冻结前问普通购电计划并从新初态重新执行储能反馈，不再重新求解；Q组在附件4电价下重新优化，其中Q43\_S0与Q43\_S2各经历$334$次$0{:}00$计划求解和$1\,002$次日内更新决策。求解仍通过SciPy接口调用HiGHS求解器，相对MIP gap目标为$10^{-9}$，单次时间上限$120$秒。整体求解流程如图~\ref{fig:q4_flow}所示。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\textwidth]{../../C题/figures/问题四求解流程/问题四求解流程.png}
  \caption{问题四滚动求解流程}
  \label{fig:q4_flow}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{../../C题/figures/q4_price_transfer/cost_components_and_savings.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{../../C题/figures/q4_paper/q4_fig1_monthly_savings_and_decomposition.png}
...
当场景中增加储能设备数量、分布式能源种类或可参与调度的柔性负荷\upcite{qi2020distribution}时，只需补充相应的决策变量和约束，即可将模型扩展为规模更大的微网综合能源调度模型。将该框架与预测更新和滚动优化相结合，还可进一步用于光伏、风电等出力不确定条件下的长期调度决策。
