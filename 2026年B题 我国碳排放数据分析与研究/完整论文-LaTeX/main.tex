...
\begin{abstract}
我国已明确提出2030年前碳达峰与2060年前碳中和目标，对碳排放的时空特征、关键驱动因素、趋势并提出对策等问题的研究具有十分重要的意义。本文针对以上问题，根据碳排放数据及国家统计年鉴中的能源结构等资料，\textbf{建立全局Moran检验与Ward层次聚类、带符号约束STIRPAT-ridge回归等模型}，对省份进行分类分级，识别碳排放驱动因素，推演2026至2045年碳排放总量与强度并判断碳达峰，最终编制分区域、分部门、分阶段的政策建议书。

\textbf{针对问题一，}我国30个省份碳排放空间差异与科学分类问题，本文构建排放规模、人均排放、排放强度、煤炭排放占比与过程排放占比五维指标体系，以反映各省碳排放的规模、效率与结构特征。通过\textbf{全局Moran检验}判断省域核心指标是否存在显著空间差异，并利用\textbf{Ward层次聚类算法}对省份进行科学分类。结果表明，人均排放、排放强度与过程排放占比呈显著空间正相关集聚，Moran指数分别为0.3145、0.4113与0.4073；基于最大轮廓系数0.3393确定最优类别数$K=3$，将30省划分为\textbf{8个高规模高压力型、21个低规模相对低碳型与1个北京结构差异簇}，并识别出9个重点治理省份。

\textbf{针对问题二，}全国碳排放宏观驱动机理识别与预测建模问题，本文构建\textbf{带符号约束STIRPAT-ridge回归模型}以识别碳排放驱动因素并建立预测模型，通过留一交叉验证优化参数，并经扩展窗口回测验证泛化能力。结果表明，\textbf{GDP}为主导正向驱动因素，相对权重78.78\%，\textbf{清洁能源比重}为核心负向抑制因素，相对权重21.22\%；样本内拟合MAE为48.034 Mt，扩展窗口外推MAE为184.397 Mt，模型兼具物理机理可解释性与定量预测能力，确立为最优预测模型。

\textbf{针对问题三，}2026至2045年三情景碳排放演进趋势与达峰特征推演问题，本文构建\textbf{多情景动态递推演化模型}，逐年预测基准、低碳与强化低碳三情景下的碳排放总量与强度，并判断碳达峰时间与峰值水平。结果表明，三情景总量均平缓上升，由低到高依次为强化低碳、低碳、基准，2045年总量分别为12890.561、13437.482与13864.762 Mt；三情景强度持续显著下降，2045年较2025年降幅达44.30\%至46.02\%；预测期内三情景均未见碳达峰，极大值出现在2045年窗口边界，\textbf{达峰时间在2045年之后}。

\textbf{针对问题四，}本文紧扣我国双碳战略目标，将前述省份分类、影响因素与情景预测结果转化为政策建议，构建涵盖\textbf{分区施策、部门攻坚与阶段推进}的政策映射模型，编制面向碳达峰与碳中和的决策建议书。建议书明确，对\textbf{8个高规模高压力型省份}全面实施总量控制、煤炭替代与能效改造；对累计排放占比达85.28\%的工业与电力部门实施深度脱碳；按筑基、攻坚、深化三阶段推进，设定2045年控制目标，使强度指数降至53.98至55.70，并配套建立年度滚动监测指标体系。
\end{abstract}
...
\subsection{问题背景}
氧化碳（CO₂）是温室气体排放的主要构成成分。其过度排放会加剧温室效应、扰乱气候系统、破坏生态环境，并对全球可持续发展构成严峻挑战。改革开放以来，我国经济高速增长的同时能源消费总量不断攀升，目前碳排放规模居全球前列。同时，我国高度重视碳减排工作，我国提出碳达峰、碳中和目标，启动全国碳排放权交易市场，并持续强调要积极稳妥推进碳达峰碳中和工作。
...
\subsection{问题一的分析}
问题一要求判断各省碳排放核心指标是否存在显著空间差异，并围绕规模、效率与经济关联度构建指标体系完成分类分级，属2022年30个省级地区的横截面识别问题。为此，本文拟按指标构建、空间检验、聚类分类与重点治理识别的路线依次展开。

指标构建方面，为覆盖题目要求的规模、效率与经济关联度三个维度，考虑到30省横截面数据量纲差异明显，本文构造排放规模、人均排放、排放强度、煤炭排放占比与过程排放占比五维指标，并经标准化处理消除量纲差异。空间检验方面，为判断核心指标是否存在显著空间差异，需借助省份间的地理邻近关系，本文构建省域邻接权重矩阵，采用全局Moran检验\upcite{moran}与置换检验判断各指标的空间集聚显著性，并针对海南无陆上相邻省份的边界情形设置桥接关系。聚类分类方面，考虑到类别数不宜主观指定，本文采用Ward层次聚类\upcite{ward}对省份分类，并通过轮廓系数在候选类别数中确定最优类别数$k=3$。重点治理识别方面，为锁定政策优先对象，以排放总量或强度处于样本前20\%的规则识别重点治理省份。
...
因素识别方面，考虑到人口、GDP、煤炭占比与清洁能源比重等候选变量之间存在明显共线性，直接回归会导致系数不稳定且方向可能违背机理，本文基于STIRPAT框架建立对数线性驱动方程，引入岭惩罚降低共线性影响，并按排放机理施加方向约束，即人口、GDP与煤炭占比系数非负、清洁能源比重系数非正。预测建模方面，考虑到正则化强度不宜主观指定，本文采用留一交叉验证选择最优参数；考虑到样本内拟合不足以证明泛化能力，采用扩展窗口回测逐年外推，并与时间趋势基线比较。数据衔接方面，考虑到2025年数据仅覆盖前三季度，本文按历史同期季节占比将其年化，得到11639.658 Mt的衔接锚点，该锚点仅用于情景预测的连续性衔接，不参与回归训练。

\subsection{问题三的分析}
问题三要求基于问题二的最优预测模型，设定基准、低碳与强化低碳三种情景，预测2026--2045年碳排放总量与强度，并判断碳达峰时间与峰值水平。考虑到问题二模型参数已确定，本问不重新估计模型，而是将固定系数嵌入情景推演框架，本文拟构建多情景动态递推演化模型，三情景的差异仅来自外生路径设定。

情景设定方面，为区分三种转型力度，本文将每种情景视作由经济增长、人口规模与能源结构三条外生路径构成：GDP按各阶段预设增速递推，人口按固定年增速递推，煤炭占比按情景步长逐年下降、清洁能源比重逐年上升，并设置占比上下限保证能源结构物理可行。衔接方面，考虑到三情景需与2025年年化锚点连续，本文引入校准系数统一尺度，再逐年计算排放总量与单位GDP排放强度。达峰判断方面，通过比较各情景预测期内总量极大值出现的位置判断碳达峰时间与峰值水平。需要说明的是，情景步长、占比上下限及校准系数均为本文依据政策力度与衔接要求设定的参数，而非题目给定常量，其取值在对应章节中说明。

\subsection{问题四的分析}
问题四要求结合前述分析与双碳目标给出建议书，其本质是把统计建模结论转化为可操作的政策语言。本文拟构建省级规则识别、类别覆盖率汇总、部门优先级排序与阶段指标监测相结合的政策映射模型。

政策工具方面，考虑到建议书需要落到具体省份，本文以省级指标样本中位数等阈值定义总量控制、能效改造、煤炭替代、市场机制与需求侧治理五类工具的可计算规则，逐省生成政策标记，规则阈值取自省级样本分布而非外部指定。治理重点方面，考虑到政策建议需按省份类别组织，本文按问题一的聚类类别汇总各类工具覆盖率，识别高规模高压力型省份的治理重点；考虑到部门减排优先级应由排放规模决定，依据各部门累计排放份额排序，聚焦工业与电力两大部门实施深度脱碳。阶段监测方面，考虑到治理目标需要可量化、可检查，本文以问题三情景强度为基础构造以2025年为基准100的强度指数，按筑基、攻坚、深化三阶段设定控制目标，并配套年度滚动监测指标体系。
...
\subsubsection{模型建立}
\paragraph{模型来源与建模思路。}
问题二以2019--2024年全国年度排放总量为响应变量，以人口、GDP、煤炭占比和清洁能源比重为驱动变量。模型来源于STIRPAT模型对IPAT恒等式的随机化、可估计扩展\upcite{stirpat}；考虑年度样本仅有6个且解释变量存在共线性，在STIRPAT框架上采用带$L_2$惩罚的岭回归\upcite{ridge}，并按排放机理施加系数方向约束。该结构用于构造方向受控的预测接口，不将短样本回归系数解释为因果效应。
