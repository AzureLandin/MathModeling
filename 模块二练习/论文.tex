...
观察180个供水点的空间分布（如图\ref{fig:spatial_dist}所示），181个节点呈现多个自然聚集区域（如左下区域、中部密集区、右上区域及顶部散布区）。这种节点之间的空间聚集性为“先聚类、再选址、后组网”的分治求解思路提供了几何直观上的支撑。
...
为削减后续最优解搜索空间大小，利用Delaunay三角剖分法对候选边进行筛选。计算几何理论表明：欧氏点集的最小生成树必为其Delaunay图的子图。这意味着利用Delaunay三角剖分法剖分后的稀疏图（约537条边）依旧包含最优管道网所需的全部候选边，删去原图中的非最优边不损失整体的最优性。图\ref{fig:delaunay}展示了Delaunay剖分结果与全局MST的叠加关系，直观验证了MST的所有边均落在剖分后的稀疏图内。
...
由于约束(\ref{eq:capacity})直接限制了簇内最小生成树的长度，本文设计了\textbf{约束下的凝聚层次聚类算法（Constrained Agglomerative Clustering）}。
\step{1} \textbf{初始化：}每个节点先自成一簇，形成180个仅含一个点的节点簇，节点簇内最小生成树长度为0。
\step{2} \textbf{距离度量：}簇间距离采用，将两个簇之间的距离确定为两个簇中任意两点之间的最短距离：$d(C_a, C_b) = \min_{i\in C_a, j\in C_b} d_{ij}$。单链接准则倾向生成链状节点簇，与管道网络的树状拓扑结构较为契合。
\step{3} \textbf{合并与校验：}反复选取距离最近的节点簇对 $(C_a, C_b)$，计算合并后候选簇 $C_{trial} = C_a \cup C_b$ 的最小生产树的总长度 $L_{trial}$。若 $L_{trial} \le 30$ km，则接受合并；否则不合并该对节点，并将这对节点标记为“不可合并”，继续尝试下一对节点。
...
\begin{itemize}
    ...
    \item \textbf{算子B：}在簇内冲洗确定一级供水站站节点，改变I型管道接入点。
    \item \textbf{算子C：}改变宏观上的节点簇结构。
\end{itemize}

所有算子生成的邻域解须严格满足30公里硬约束，不可行解直接丢弃。接受准则采用Metropolis准则，用于确定是否接受新的解：$P = \exp(-\Delta C / T)$。经过5次多重搜索，算法收敛至全局最优。
...
逐簇检查II型管道MST长度，结果如图\ref{fig:cluster_length}所示。全部17个簇的最小生成树长度均严格低于30公里上限。大部分簇的容量利用率在86\%-99\%之间（如C17利用率达99.8\%），说明聚类算法在满足硬约束的同时充分利用了每簇的长度配额。C1（P125独立成簇）利用率低，是因为P125为空间孤立点，独立成簇反而使总成本更低。
