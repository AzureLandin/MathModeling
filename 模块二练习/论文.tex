...
\noindent\zihao{5}本附录给出模型求解过程中使用的核心程序。代码按“数据预处理—约束聚类—一级站选址—两级组网—模拟退火优化—结果可视化”的执行顺序编排；为控制篇幅，仅保留关键函数与主要调用关系。\par

\begin{lstlisting}[caption={\texttt{main.py}：主程序入口与结果输出}]
# -*- coding: utf-8 -*-
"""B题 管道最优铺设问题 -- 主程序"""
...
\begin{lstlisting}[caption={\texttt{data\_loader.py}：坐标读取与空间数据预处理}]
# -*- coding: utf-8 -*-
"""第一步：数据读取与距离矩阵计算"""
import numpy as np
...
\begin{lstlisting}[caption={\texttt{clustering.py}：30公里约束凝聚聚类}]
# -*- coding: utf-8 -*-
...
\begin{lstlisting}[caption={\texttt{station\_selection.py}：一级供水站选址}]
# -*- coding: utf-8 -*-
"""第三步：加权评分一级站选址"""
import numpy as np
...
\begin{lstlisting}[caption={\texttt{network\_design.py}：两级管网生成与成本核算}]
# -*- coding: utf-8 -*-
"""第四步：管网生成与总成本核算"""
import numpy as np
...
\begin{lstlisting}[caption={\texttt{optimization.py}：模拟退火与邻域搜索}]
# -*- coding: utf-8 -*-
"""第五步：模拟退火优化"""
...
\begin{lstlisting}[caption={\texttt{utils.py}：距离矩阵与最小生成树工具}]
# -*- coding: utf-8 -*-
"""公共工具模块：MST计算、距离矩阵"""
import numpy as np
...
\begin{lstlisting}[caption={\texttt{visualization.py}：论文图表生成}]
# -*- coding: utf-8 -*-
"""
可视化模块：生成论文所需8张图
"""
...
def plot_fig1(coords):
    ...
    ax.scatter(coords[1:, 0], coords[1:, 1],
        c='#3498DB', s=30, zorder=3,
        label='供水点 (P1-P180)')
    ax.scatter(coords[0, 0], coords[0, 1],
        c='#E74C3C', s=300, marker='*', zorder=5,
        label='中心供水站 A')
    ax.annotate('A (26, 31)', xy=(26, 31),
        xytext=(28, 28), fontsize=14,
    ...
    plt.tight_layout()
    plt.savefig(f'{FIGURES_DIR}/图1_供水点空间分布.png',
                dpi=300, bbox_inches='tight')
    plt.savefig(f'{FIGURES_DIR}/图1_供水点空间分布.pdf',
                bbox_inches='tight')
    plt.close()
    print("  图1 已保存")
...
def plot_fig2(coords, dist_matrix):
            ...
            for j in range(i + 1, 3):
                ...
                if edge not in edges_plotted:
                    ...
                    ax.plot([coords[a, 0], coords[b, 0]],
                        ...
                        c='#BDC3C7', linewidth=0.5,
                        zorder=1)

    # 全局MST
    _, mst_edges = compute_mst(
        set(range(181)), dist_matrix)
    for (u, v) in mst_edges:
        ax.plot([coords[u, 0], coords[v, 0]],
            [coords[u, 1], coords[v, 1]],
            c='#E67E22', linewidth=1.5, alpha=0.8,
            zorder=2)

    ax.scatter(coords[1:, 0], coords[1:, 1],
        c='#3498DB', s=15, zorder=3)
    ax.scatter(coords[0, 0], coords[0, 1],
        c='#E74C3C', s=200, marker='*', zorder=5)

    legend_elements = [
        Line2D([0], [0], color='#BDC3C7',
            linewidth=1,
            label=f'Delaunay候选边 ({len(edges_plotted)}条)'),
        Line2D([0], [0], color='#E67E22',
            linewidth=2,
            label=f'全局MST ({len(mst_edges)}条)'),
    ]
    ...
    plt.tight_layout()
    plt.savefig(f'{FIGURES_DIR}/图2_Delaunay三角剖分.png',
                dpi=300, bbox_inches='tight')
    plt.savefig(f'{FIGURES_DIR}/图2_Delaunay三角剖分.pdf',
                bbox_inches='tight')
    plt.close()
    print("  图2 已保存")


def plot_fig3(coords, clusters, primary_stations,
    ...
    for k in range(K):
        ...
        for (u, v) in cluster_mst_edges[k]:
            ax.plot([coords[u, 0], coords[v, 0]],
                [coords[u, 1], coords[v, 1]],
                c=color, linewidth=1.0, alpha=0.7,
                zorder=2)
        ...
        ax.scatter(coords[ps, 0], coords[ps, 1],
            c='black', s=120, marker='s', zorder=5,
            edgecolors='white', linewidths=1.5)

        centroid = coords[nodes].mean(axis=0)
        ax.annotate(f'C{k+1}\n({cluster_mst_lengths[k]:.1f}km)',
            xy=centroid, fontsize=11, ha='center',
            color=color, fontweight='bold')

    ax.scatter(coords[0, 0], coords[0, 1],
        c='#E74C3C', s=300, marker='*', zorder=6)
    ax.annotate('A', xy=(26, 31), xytext=(27, 29),
    ...
    ax.grid(True, alpha=0.2)
    plt.tight_layout()
    plt.savefig(f'{FIGURES_DIR}/图3_聚类结果.png',
                dpi=300, bbox_inches='tight')
    plt.savefig(f'{FIGURES_DIR}/图3_聚类结果.pdf',
                bbox_inches='tight')
    plt.close()
    print("  图3 已保存")
...
def plot_fig4(coords, E_I, primary_stations,
    ...
    ax.scatter(coords[1:, 0], coords[1:, 1],
        c='#ECF0F1', s=15, zorder=1)

    for (u, v) in E_I:
        ax.plot([coords[u, 0], coords[v, 0]],
            [coords[u, 1], coords[v, 1]],
            c='#E74C3C', linewidth=2.5, zorder=3,
            solid_capstyle='round')
        mid = (coords[u] + coords[v]) / 2
        length = dist_matrix[u][v]
        ax.annotate(f'{length:.1f}', xy=mid,
    ...
    for ps in primary_stations:
        ax.scatter(coords[ps, 0], coords[ps, 1],
            c='#2C3E50', s=150, marker='s', zorder=5,
            edgecolors='white', linewidths=1.5)
        ax.annotate(f'P{ps}',
            xy=(coords[ps, 0], coords[ps, 1]),
            xytext=(5, 5), textcoords='offset points',
            fontsize=11, fontweight='bold')

    ax.scatter(coords[0, 0], coords[0, 1],
        c='#E74C3C', s=350, marker='*', zorder=6)
    ax.annotate('A', xy=(26, 31), xytext=(27, 29),
    ...
    ax.grid(True, alpha=0.2)
    plt.tight_layout()
    plt.savefig(f'{FIGURES_DIR}/图4_I型管网.png',
                dpi=300, bbox_inches='tight')
    plt.savefig(f'{FIGURES_DIR}/图4_I型管网.pdf',
                bbox_inches='tight')
    plt.close()
    print("  图4 已保存")


def plot_fig5(coords, E_I, E_II, primary_stations,
              L_I, L_II, C_total):
    """图5：完整管网拓扑图（最终方案）"""
    ...
    for edges in E_II:
        for (u, v) in edges:
            ax.plot([coords[u, 0], coords[v, 0]],
                [coords[u, 1], coords[v, 1]],
                c='#3498DB', linewidth=1.0, alpha=0.8,
                zorder=2)
    ...
    # 二级站
    T_nodes = list(set(range(1, 181)) -
                   set(primary_stations))
    ax.scatter(coords[T_nodes, 0], coords[T_nodes, 1],
        c='#3498DB', s=25, zorder=4,
        edgecolors='white', linewidths=0.3)

    # 一级站
    for ps in primary_stations:
        ax.scatter(coords[ps, 0], coords[ps, 1],
            c='#2C3E50', s=130, marker='s', zorder=5,
            edgecolors='white', linewidths=1.5)
    ...
    legend_elements = [
        Line2D([0], [0], color='#E74C3C',
            linewidth=2.5,
            label=f'I型管道 ({L_I:.2f} km)'),
        Line2D([0], [0], color='#3498DB',
            linewidth=1.0,
            label=f'II型管道 ({L_II:.2f} km)'),
        Line2D([0], [0], marker='*', color='w',
            markerfacecolor='#E74C3C', markersize=15,
            label='中心站 A'),
        Line2D([0], [0], marker='s', color='w',
        ...
        Line2D([0], [0], marker='o', color='w',
            markerfacecolor='#3498DB', markersize=7,
            label='二级供水站'),
    ]
    ax.legend(handles=legend_elements, loc='upper left')
    ax.set_xlabel('X 坐标 (km)')
    ...
    ax.grid(True, alpha=0.2)
    plt.tight_layout()
    plt.savefig(f'{FIGURES_DIR}/图5_完整管网拓扑.png',
                dpi=300, bbox_inches='tight')
    plt.savefig(f'{FIGURES_DIR}/图5_完整管网拓扑.pdf',
                bbox_inches='tight')
    plt.close()
    print("  图5 已保存")
...
def plot_fig6(cluster_mst_lengths):
    ...
    bars = ax.bar(x, cluster_mst_lengths,
        color=plt.cm.Blues(
            np.linspace(0.4, 0.9, K)),
        edgecolor='white', linewidth=0.5)
    ax.axhline(y=30.0, color='#E74C3C',
    ...
    plt.tight_layout()
    plt.savefig(f'{FIGURES_DIR}/图6_各簇管道长度.png',
                dpi=300, bbox_inches='tight')
    plt.savefig(f'{FIGURES_DIR}/图6_各簇管道长度.pdf',
                bbox_inches='tight')
    plt.close()
    print("  图6 已保存")
...
def plot_fig7(convergence_curve, initial_cost):
    ...
    ax.scatter([iterations[0]], [costs[0]],
        c='#E74C3C', s=80, zorder=5,
        label=f'初始解: {costs[0]:.2f} 万元')
    ax.scatter([iterations[-1]], [costs[-1]],
        c='#27AE60', s=80, zorder=5,
    ...
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f'{FIGURES_DIR}/图7_收敛曲线.png',
                dpi=300, bbox_inches='tight')
    plt.savefig(f'{FIGURES_DIR}/图7_收敛曲线.pdf',
                bbox_inches='tight')
    plt.close()
    print("  图7 已保存")
...
def plot_fig8(L_I, L_II, C_total):
    ...
    ax1.pie(type_costs, labels=type_labels,
    ...
    detail_labels = ['I型材料费\n(571元/m)',
        ...
        'II型材料费\n(235元/m)',
        'II型铺设费\n(210元/m)']
    detail_costs = [571000 * L_I, 720000 * L_I,
        235000 * L_II, 210000 * L_II]
    colors_detail = ['#E74C3C', '#F1948A',
                     '#3498DB', '#85C1E9']
    ax2.pie(detail_costs, labels=detail_labels,
        colors=colors_detail, autopct='%1.1f%%',
        startangle=90, textprops={'fontsize': 13})
    ax2.set_title('按费用性质', fontsize=16)

    plt.tight_layout()
    plt.savefig(f'{FIGURES_DIR}/图8_成本构成.png',
                dpi=300, bbox_inches='tight')
    plt.savefig(f'{FIGURES_DIR}/图8_成本构成.pdf',
                bbox_inches='tight')
    plt.close()
    print("  图8 已保存")
...
def generate_all_figures(coords, dist_matrix,
    ...
    plot_fig3(coords, clusters, primary_stations,
        cluster_mst_lengths, cluster_mst_edges)
    plot_fig4(coords, E_I, primary_stations,
        L_I, dist_matrix)
    plot_fig5(coords, E_I, E_II, primary_stations,
        L_I, L_II, C_total)
